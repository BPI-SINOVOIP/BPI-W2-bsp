...
static const struct RTK_pinctrl_desc rtk16xx_pinctrl_data = {
	.pins = rtk_pins,
	...
	enum pin_config_param arg)
{
	...
	case PIN_CONFIG_INPUT_SCHMITT_ENABLE:
		...
		writel(((val & ~mask)|
			(set_val <<
			(pin_regmap[pin].pcof_regbit + pconf_schm))),
			addr);
		break;
	case PIN_CONFIG_DRIVE_STRENGTH:
		...
		writel(((val & ~mask)|
			(set_val <<
			(pin_regmap[pin].pcof_regbit + pconf_curr))),
			addr);
		break;
	case PIN_CONFIG_DRIVE_PUSH_PULL:
		...
		writel(((val & ~mask)|
			(1 << (pin_regmap[pin].pcof_regbit + pconf_pulen))),
			addr);
		break;
	case PIN_CONFIG_BIAS_DISABLE:
		...
		mask = 1 << (pin_regmap[pin].pcof_regbit + pconf_pulen);
		writel(((val & ~mask)|
			(0 << (pin_regmap[pin].pcof_regbit + pconf_pulen))),
			addr);
		break;
	case PIN_CONFIG_BIAS_PULL_UP:
		if (pin == 52) {
			...
			writel(((val & ~mask)|
				(1 << (pin_regmap[pin].pcof_regbit + pconf_pulsel))),
				addr);
		}
		break;
	case PIN_CONFIG_BIAS_PULL_DOWN:
		if (pin == 52) {
			...
			mask =	1 << (pin_regmap[pin].pcof_regbit + pconf_pulsel);
			writel(((val & ~mask)|
				(0 << (pin_regmap[pin].pcof_regbit + pconf_pulsel))),
				addr);
		}
		break;
	default:
...
static const struct of_device_id RTK_pinctrl_match[] = {
	{ .compatible = "realtek,rtk16xx-pinctrl",
		.data = (void *)&rtk16xx_pinctrl_data },
	{ /* Sentinel */ },
};
...
static int RTK_pinctrl_probe(struct platform_device *pdev)
{
	...
	pctl->misc_membase = of_iomap(node, 1);
	if (!pctl->misc_membase) {
		RTK_PINCTRL_ERR("of_iomap misc_membase fail");
		return -ENOMEM;
	}

	RTK_PINCTRL_DBG("misc_membase(0x%p)", pctl->misc_membase);
