...
DECLARE_BITMAP(rtk_pcie0_irq_bitmap, MAX_RTK_MSI_IRQS);
unsigned int *pcie_msi_data;
dma_addr_t dma_handle;

#endif


static void rtk_pcie_13xx_ctrl_write(unsigned long addr, unsigned int val)
{
	writel(val, addr + PCIE_CTRL_BASE);
}

static unsigned int rtk_pcie_13xx_ctrl_read(unsigned long addr)
{
	unsigned int val = readl(addr + PCIE_CTRL_BASE);
	return val;
}

static void rtk_pcie_13xx_direct_cfg_write(unsigned long addr, unsigned int val)
{
	writel(val, addr + PCIE_CFG_BASE);
}

static void rtk_pcie_13xx_direct_cfg_write_word(unsigned long addr, u16 val)
{
	writew(val, addr + PCIE_CFG_BASE);
}

static void rtk_pcie_13xx_direct_cfg_write_byte(unsigned long addr, u8 val)
{
	writeb(val, addr + PCIE_CFG_BASE);
}

static unsigned int rtk_pcie_13xx_direct_cfg_read(unsigned long addr)
{
	unsigned int val = readl(addr + PCIE_CFG_BASE);
	return val;
}

static u16 rtk_pcie_13xx_direct_cfg_read_word(unsigned long addr)
{
	u16 val = readw(addr + PCIE_CFG_BASE);
	return val;
}

static u8 rtk_pcie_13xx_direct_cfg_read_byte(unsigned long addr)
{
...
u32 rtk_pcie_13xx_mmio_start(void)
{
	return PCIE_MMIO_PHY_ADDR;
}

u32 rtk_pcie_13xx_mmio_end(void)
{
	return (PCIE_MMIO_PHY_ADDR + (PCIE_MMIO_PHY_ADDR_LEN - 1));
}

u32 rtk_pcie_13xx_mmio_len(void)
{
...
u32 rtk_pcie_13xx_read(u32 addr, u8 size)
{
	...
	spin_lock_irqsave(&rtk_pcie_13xx_lock, irqL);
	...
	if (tmp_addr == 0xCEC || tmp_addr == 0xC68) {
		...
		rtk_pcie_13xx_ctrl_write(0xD00, PCIE_IO_2K_MASK);
		translate_val = rtk_pcie_13xx_ctrl_read(0xD04);
		rtk_pcie_13xx_ctrl_write(0xD04, translate_val | (addr & mask));
...
pci_read_13xx_retry:
	...
	switch (size) {
	case 1:
		rval = rtk_pcie_13xx_direct_cfg_read_byte((addr & ~mask));
		break;
	case 2:
		rval = rtk_pcie_13xx_direct_cfg_read_word((addr & ~mask));
		break;
	case 4:
		rval = rtk_pcie_13xx_direct_cfg_read((addr & ~mask));
		break;
	default:
	...
	//DLLP error patch
	pci_error_status = rtk_pcie_13xx_ctrl_read(0xc7c);
	if (pci_error_status & 0x1F) {
		rtk_pcie_13xx_ctrl_write(0xc7c, pci_error_status);
	 ...
	 */
	if (tmp_addr == 0xCEC || tmp_addr == 0xC68) {
		rtk_pcie_13xx_ctrl_write(0xD04, translate_val);
		rtk_pcie_13xx_ctrl_write(0xD00, PCIE_IO_64K_MASK);
	} else if (addr >= 0x10000) {
		rtk_pcie_13xx_ctrl_write(0xD04, translate_val);
	}

	spin_unlock_irqrestore(&rtk_pcie_13xx_lock, irqL);
...
void rtk_pcie_13xx_write(u32 addr, u8 size, u32 wval)
{
	...
	switch (size) {
	case 1:
		rtk_pcie_13xx_direct_cfg_write_byte((addr&~mask), wval);
		break;
	case 2:
		rtk_pcie_13xx_direct_cfg_write_word((addr&~mask), wval);
		break;
	case 4:
		rtk_pcie_13xx_direct_cfg_write((addr&~mask), wval);
		break;
	default:
		pr_info("RTD13xx: %s: wrong size %d\n", __func__, size);
		break;
					...
					unsigned char size)
{
	...
	rtk_pcie_13xx_ctrl_write(PCIE_INDIR_CTR, 0x12);
	rtk_pcie_13xx_ctrl_write(PCIE_CFG_ST, CFG_ST_ERROR|CFG_ST_DONE);
	rtk_pcie_13xx_ctrl_write(PCIE_CFG_ADDR, addr);
	rtk_pcie_13xx_ctrl_write(PCIE_CFG_WDATA, data);

	if (size == 4)
		rtk_pcie_13xx_ctrl_write(PCIE_CFG_EN, 0x1);
	else
		rtk_pcie_13xx_ctrl_write(PCIE_CFG_EN,
			BYTE_CNT(mask) | BYTE_EN | WRRD_EN(1));

	rtk_pcie_13xx_ctrl_write(PCIE_CFG_CT, GO_CT);

	do {
		status = rtk_pcie_13xx_ctrl_read(PCIE_CFG_ST);
		udelay(50);
	...
	if (rtk_pcie_13xx_ctrl_read(PCIE_CFG_ST) & CFG_ST_ERROR) {
						...
						unsigned char size)
{
	...
	rtk_pcie_13xx_ctrl_write(PCIE_INDIR_CTR, 0x10);
	rtk_pcie_13xx_ctrl_write(PCIE_CFG_ST, 0x3);
	rtk_pcie_13xx_ctrl_write(PCIE_CFG_ADDR, (addr & ~0x3));
	rtk_pcie_13xx_ctrl_write(PCIE_CFG_EN,
	...
	*pdata = (rtk_pcie_13xx_ctrl_read(PCIE_CFG_RDATA) & _pci_bit_mask(mask))
							>> _pci_bit_shift(addr);
	return PCIBIOS_SUCCESSFUL;
...
static int rtk_pcie_13xx_rd_conf(struct pci_bus *bus, unsigned int devfn,
...
again:
	if (bus->number == 1 && PCI_SLOT(devfn) == 0 && PCI_FUNC(devfn) == 0) {
		if (cfg_direct_access) {
			...
			rtk_pcie_13xx_ctrl_write(0xD00, 0xFFFFF000);

			if (size == 1)
				*pval = rtk_pcie_13xx_direct_cfg_read_byte(reg);
			else if (size == 2)
				*pval = rtk_pcie_13xx_direct_cfg_read_word(reg);
			else if (size == 4)
				*pval = rtk_pcie_13xx_direct_cfg_read(reg);

			rtk_pcie_13xx_ctrl_write(0xD00, 0xFFF00000);
	...
	val = rtk_pcie_13xx_ctrl_read(0xC7C);
	if ((val & 0x1f) && retry) {
		rtk_pcie_13xx_ctrl_write(0xC7C, val);
		retry--;
		dev_err(&bus->dev, "pcie dllp error occur = 0x%x\n", val);
		goto again;
	}

	//dev_info(&bus->dev, "rtk_pcie_13xx_rd_conf reg = 0x%x, devfun:%d, size=%d, *pval = 0x%x\n",
	//							reg, devfn, size, *pval);

	return ret;
}

static int rtk_pcie_13xx_wr_conf(struct pci_bus *bus, unsigned int devfn,
			     int reg, int size, u32 val)
{
	unsigned long address;
	int ret = PCIBIOS_DEVICE_NOT_FOUND;

	//dev_info(&bus->dev, "rtk_pcie_13xx_wr_conf reg = 0x%x, val = 0x%x\n",
	//							reg, val);
	...
	if (bus->number == 1 && PCI_SLOT(devfn) == 0 && PCI_FUNC(devfn) == 0) {
		...
		if (cfg_direct_access) {
			...
			if (size == 1)
				rtk_pcie_13xx_direct_cfg_write_byte(reg, val);
			else if (size == 2)
				rtk_pcie_13xx_direct_cfg_write_word(reg, val);
			else if (size == 4)
				rtk_pcie_13xx_direct_cfg_write(reg, val);
...
static struct pci_ops rtk_pcie_13xx_ops = {
	.read = rtk_pcie_13xx_rd_conf,
	.write = rtk_pcie_13xx_wr_conf,
};

static int rtk_pcie_13xx_hw_initial(struct device *dev)
{
	...
	/*USB PCIE PHY Selector (select to PCIE)*/
	writel(readl(USB_PCIE_PHY) & ~0x40, USB_PCIE_PHY);
	...
	rtk_pcie_13xx_ctrl_write(0xC00, 0x00140010);

	if (speed_mode == 0) {
		rtk_pcie_13xx_ctrl_write(0x0A0,
			(rtk_pcie_13xx_ctrl_read(0x0A0)&0xFFFFFFF0)|0x00000001);
	}
	...
	for (i = 0; i < size; i++) {
		rtk_pcie_13xx_ctrl_write(0xC1C, phy[i]);
		mdelay(1);
	}
	kfree(phy);
	...
	/* set to MMIO */
	if (cfg_direct_access)
		rtk_pcie_13xx_ctrl_write(0xC00, 0x00BE0002);
	else
		rtk_pcie_13xx_ctrl_write(0xC00, 0x001E0022);

	mdelay(50);

	/* #Link initial setting */
	rtk_pcie_13xx_ctrl_write(0x710, 0x00010120);

	do {
		pci_link_detected = (rtk_pcie_13xx_ctrl_read(0xCB4) & 0x800);
	...
	if (pci_link_detected) {
		...
		if (!debug_mode) { /*do not turn off clk in debug mode*/
			/*USB PCIE PHY Selector (select to USB)*/
			writel(readl(USB_PCIE_PHY) | 0x40, USB_PCIE_PHY);
	...
	rtk_pcie_13xx_ctrl_write(0x04, 0x00000007);

	if (cfg_direct_access) {
		/* #Base 1 */
		rtk_pcie_13xx_ctrl_write(0xD08, 0x98061000);

		/* #Mask 1 */
		rtk_pcie_13xx_ctrl_write(0xD0C, 0xFFFFF000);

		/* #translate for CFG R/W */
		rtk_pcie_13xx_ctrl_write(0xD10, 0x00000000);
	}
	/* #Base 0 */
	rtk_pcie_13xx_ctrl_write(0xCFC, 0x98062000);

	/* #Mask 0 */
	rtk_pcie_13xx_ctrl_write(0xD00, 0xFFF00000);

	/* #translate for MMIO R/W */
	rtk_pcie_13xx_ctrl_write(0xD04, 0x98000000);

#if defined(CONFIG_R8125) || defined(CONFIG_R8125_MODULE)
	/*pcie timeout extend to 500us*/
	rtk_pcie_13xx_ctrl_write(0xC78, 0xF42401);
#else
	/* prevent pcie hang if dllp error occur*/
	rtk_pcie_13xx_ctrl_write(0xC78, 0x200001);
#endif
	/* set limit and base register */
	rtk_pcie_13xx_ctrl_write(0x20, 0x0000FFF0);
	rtk_pcie_13xx_ctrl_write(0x24, 0x0000FFF0);

#ifdef CONFIG_PCI_MSI
	rtk_pcie_13xx_ctrl_write(0xC04, 0x4C00);
#endif
...
irqreturn_t rtk_handle_msi_irq(int this_irq, void *dev_id)
{
	...
	if (rtk_pcie_13xx_ctrl_read(0xCD4) & 0x10000) {
		rtk_pcie_13xx_ctrl_write(0xCD4, 0x10000);
	} else {
		return IRQ_HANDLED;
	}
	...
	while ((pos = find_next_bit(&val, MAX_RTK_MSI_IRQS, pos)) != MAX_RTK_MSI_IRQS) {
		irq = irq_find_mapping(rtk_msi_domain, pos);
...
{
	pcie_msi_data = __get_free_pages(GFP_KERNEL, 0);
	rtk_pcie_13xx_ctrl_write(0xCD0, virt_to_phys((void *)pcie_msi_data));
	*(volatile unsigned int *)pcie_msi_data = 0;
}
...
static int rtk_msi_alloc(void)
{
	...
	mutex_lock(&msi_lock);
	msi = find_first_zero_bit(rtk_pcie0_irq_bitmap, MAX_RTK_MSI_IRQS);
	if (msi < MAX_RTK_MSI_IRQS)
		set_bit(msi, rtk_pcie0_irq_bitmap);
	else
		msi = -ENOSPC;
...
static void rtk_msi_free(unsigned long irq)
{
	...
	if (!test_bit(irq, rtk_pcie0_irq_bitmap))
		pr_err("[%s]trying to free unused MSI#%lu\n", __func__, irq);
	else
		clear_bit(irq, rtk_pcie0_irq_bitmap);
			       ...
			       struct pci_dev *pdev, struct msi_desc *desc)
{
	...
	irq = irq_create_mapping(rtk_msi_domain, hwirq);
	if (!irq) {
...
static int rtk_pcie_13xx_probe(struct platform_device *pdev)
{
	int ret = 0;
	int size = 0;
	...
	spin_lock_init(&rtk_pcie_13xx_lock);
	...
	USB_PCIE_PHY = of_iomap(pdev->dev.of_node, 3);
	if (!USB_PCIE_PHY) {
		dev_err(&pdev->dev, "pcie no USB_PCIE_PHY address\n");
		return -EINVAL;
	}
	...
	if (gpio_is_valid(pcie_gpio)) {
		ret = gpio_request(pcie_gpio, "pcie_gpio");
		if (ret < 0)
	...
	if (rtk_pcie_13xx_hw_initial(&pdev->dev) < 0) {
		dev_err(&pdev->dev, "rtk_pcie_13xx_hw_initial fail\n");
		return -EINVAL;
	}
	...
	if (irq < 0) {
		pr_err("PCIE0 parse irq(%d) failed\n", irq);
		return -EINVAL;
	}
	ret = request_irq(irq, rtk_handle_msi_irq, IRQF_SHARED, "rtk-pcie1-msi", pdev);
	if (ret !=0)
		pr_err("[%s]request irq failed\n", __func__);
	rtk_msi_domain = irq_domain_add_linear(NULL, MAX_RTK_MSI_IRQS, &rtk_msi_domain_ops, &rtk_pcie_msi_chip);
	if (!rtk_msi_domain) {
		dev_err(&pdev->dev, "irq domain init failed\n");
		return -ENXIO;
	}
	pcie_msi_data = dma_alloc_coherent(&pdev->dev, PAGE_SIZE, &dma_handle, GFP_KERNEL);
	rtk_pcie_13xx_ctrl_write(0xCD0, dma_handle);
	*pcie_msi_data = 0x0;
#endif
...
static int rtk_pcie_13xx_suspend(struct device *dev)
{
	dev_info(dev, "suspend enter ...\n");

	if (RTK_PM_STATE == PM_SUSPEND_STANDBY) {
		rtk_pcie_13xx_ctrl_write(0x178, 0xA3FF0001);
		rtk_pcie_13xx_ctrl_write(0x098, 0x400);
		rtk_pcie_13xx_ctrl_write(0xC6C, 0x00000031);
...
static int rtk_pcie_13xx_resume(struct device *dev)
{
	int ret = 0;
	...
	if (RTK_PM_STATE == PM_SUSPEND_STANDBY) {
		rtk_pcie_13xx_ctrl_write(0xC6C, 0x00000032);
		dev_info(dev, "Idle mode\n");
	} else {
		dev_info(dev, "Suspend mode\n");
		if (gpio_is_valid(pcie_gpio)) {
		...
		if (rtk_pcie_13xx_hw_initial(dev) < 0) {
			dev_err(dev, "rtk_pcie_13xx_hw_initial fail\n");
			return -EINVAL;
		}
...
static void rtk_pcie_13xx_shutdown(struct platform_device *pdev)
{
...
static const struct dev_pm_ops rtk_pcie_13xx_pm_ops = {
	.suspend_noirq = rtk_pcie_13xx_suspend,
	.resume_noirq = rtk_pcie_13xx_resume,
};
#endif //CONFIG_SUSPEND
static const struct of_device_id rtk_pcie_13xx_match_table[] = {
	{.compatible = "realtek,rtk13xx-pcie-slot1",},
	{},
};

static struct platform_driver rtk_pcie_13xx_driver = {
	.driver = {
		.name = "[RTD13xx PCIE1]",
		.of_match_table = of_match_ptr(rtk_pcie_13xx_match_table),
#ifdef CONFIG_SUSPEND
		.pm = &rtk_pcie_13xx_pm_ops,
#endif
	},
	.probe = rtk_pcie_13xx_probe,
	.shutdown = rtk_pcie_13xx_shutdown,
};
module_platform_driver(rtk_pcie_13xx_driver);
