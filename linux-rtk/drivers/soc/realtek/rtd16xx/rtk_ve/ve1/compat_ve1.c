...
#ifdef SUPPORT_MULTI_INST_INTR
	err |= get_user(i, &data32->intr_inst_index);
	err |= put_user(i, &data->intr_inst_index);
#endif
...
static int compat_get_ve1_dovi_info(
	compat_vpudrv_dovi_info_t __user *data32,
	vpudrv_dovi_info_t __user *data)
{
	compat_uint_t c;
	compat_uint_t e;
	compat_uint_t i;
...
static int compat_put_ve1_dovi_info(
	compat_vpudrv_dovi_info_t __user *data32,
	vpudrv_dovi_info_t __user *data)
{
	compat_uint_t c;
...
long compat_vpu_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_VDI_IOCTL_SET_RTK_DOVI_FLAG:
	{
		compat_vpudrv_dovi_info_t __user *data32;
		vpudrv_dovi_info_t __user *data;
		int err;
		...
		err = compat_get_ve1_dovi_info(data32, data);
		if (err)
			return err;
		ret = filp->f_op->unlocked_ioctl(filp, VDI_IOCTL_SET_RTK_DOVI_FLAG, (unsigned long)data);
		err = compat_put_ve1_dovi_info(data32, data);
		return ret ? ret : err;
	}
	...
	case COMPAT_VDI_IOCTL_GET_RTK_DCSYS_INFO:
	{
		...
		if (err)
			return err;
		ret = filp->f_op->unlocked_ioctl(filp, VDI_IOCTL_GET_RTK_DCSYS_INFO, (unsigned long)data);
		err = compat_put_ve1_buffer_data(data32, data);
		return ret ? ret : err;
	}
	...
	case COMPAT_VDI_IOCTL_GET_TOTAL_INSTANCE_NUM:
	{
		...
		if (err)
			return err;
		ret = filp->f_op->unlocked_ioctl(filp, VDI_IOCTL_GET_TOTAL_INSTANCE_NUM, (unsigned long)data);
		err = compat_put_ve1_inst_info_data(data32, data);
		return ret ? ret : err;
	}
