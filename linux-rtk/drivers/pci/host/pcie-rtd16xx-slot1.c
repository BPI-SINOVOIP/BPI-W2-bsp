...
static void rtk_pcie_16xx_ctrl_write(unsigned long addr, unsigned int val)
{
	writel(val, addr + PCIE_CTRL_BASE);
}

static unsigned int rtk_pcie_16xx_ctrl_read(unsigned long addr)
{
	unsigned int val = readl(addr + PCIE_CTRL_BASE);
	return val;
}

static void rtk_pcie_16xx_direct_cfg_write(unsigned long addr, unsigned int val)
{
	writel(val, addr + PCIE_CFG_BASE);
}

static void rtk_pcie_16xx_direct_cfg_write_word(unsigned long addr, u16 val)
{
	writew(val, addr + PCIE_CFG_BASE);
}

static void rtk_pcie_16xx_direct_cfg_write_byte(unsigned long addr, u8 val)
{
	writeb(val, addr + PCIE_CFG_BASE);
}

static unsigned int rtk_pcie_16xx_direct_cfg_read(unsigned long addr)
{
	unsigned int val = readl(addr + PCIE_CFG_BASE);
	return val;
}

static u16 rtk_pcie_16xx_direct_cfg_read_word(unsigned long addr)
{
	u16 val = readw(addr + PCIE_CFG_BASE);
	return val;
}

static u8 rtk_pcie_16xx_direct_cfg_read_byte(unsigned long addr)
{
...
u32 rtk_pcie_16xx_mmio_end(void)
{
	return (PCIE_MMIO_PHY_ADDR + (PCIE_MMIO_PHY_ADDR_LEN - 1));
}

u32 rtk_pcie_16xx_mmio_len(void)
{
	return PCIE_MMIO_PHY_ADDR_LEN;
}

u32 rtk_pcie_16xx_read(u32 addr, u8 size)
{
	u32 rval = 0;
	u32 mask;
	u32 translate_val = 0;
	...
	spin_lock_irqsave(&rtk_pcie_16xx_lock, irqL);
	...
	if (tmp_addr == 0xCEC || tmp_addr == 0xC68) {
		...
		rtk_pcie_16xx_ctrl_write(0xD00, PCIE_IO_2K_MASK);
		translate_val = rtk_pcie_16xx_ctrl_read(0xD04);
		rtk_pcie_16xx_ctrl_write(0xD04, translate_val | (addr & mask));
...
pci_read_16xx_retry:
	...
	switch (size) {
	case 1:
		rval = rtk_pcie_16xx_direct_cfg_read_byte((addr & ~mask));
		break;
	case 2:
		rval = rtk_pcie_16xx_direct_cfg_read_word((addr & ~mask));
		break;
	case 4:
		rval = rtk_pcie_16xx_direct_cfg_read((addr & ~mask));
		break;
	default:
	...
	//DLLP error patch
	pci_error_status = rtk_pcie_16xx_ctrl_read(0xc7c);
	if (pci_error_status & 0x1F) {
		rtk_pcie_16xx_ctrl_write(0xc7c, pci_error_status);
	 ...
	 */
	if (tmp_addr == 0xCEC || tmp_addr == 0xC68) {
		rtk_pcie_16xx_ctrl_write(0xD04, translate_val);
		rtk_pcie_16xx_ctrl_write(0xD00, PCIE_IO_64K_MASK);
	} else if (addr >= 0x10000) {
		rtk_pcie_16xx_ctrl_write(0xD04, translate_val);
	}

	spin_unlock_irqrestore(&rtk_pcie_16xx_lock, irqL);

	return rval;
}
EXPORT_SYMBOL(rtk_pcie_16xx_read);

void rtk_pcie_16xx_write(u32 addr, u8 size, u32 wval)
{
	u32 mask;
	u32 translate_val = 0;
	u32 tmp_addr = addr & 0xFFF;
	...
	switch (size) {
	case 1:
		rtk_pcie_16xx_direct_cfg_write_byte((addr&~mask), wval);
		break;
	case 2:
		rtk_pcie_16xx_direct_cfg_write_word((addr&~mask), wval);
		break;
	case 4:
		rtk_pcie_16xx_direct_cfg_write((addr&~mask), wval);
		break;
	default:
		pr_info("RTD16xx: %s: wrong size %d\n", __func__, size);
		break;
...
EXPORT_SYMBOL(rtk_pcie_16xx_write);
					...
					unsigned char size)
{
	...
	rtk_pcie_16xx_ctrl_write(PCIE_INDIR_CTR, 0x12);
	rtk_pcie_16xx_ctrl_write(PCIE_CFG_ST, CFG_ST_ERROR|CFG_ST_DONE);
	rtk_pcie_16xx_ctrl_write(PCIE_CFG_ADDR, addr);
	rtk_pcie_16xx_ctrl_write(PCIE_CFG_WDATA, data);

	if (size == 4)
		rtk_pcie_16xx_ctrl_write(PCIE_CFG_EN, 0x1);
	else
		rtk_pcie_16xx_ctrl_write(PCIE_CFG_EN,
			BYTE_CNT(mask) | BYTE_EN | WRRD_EN(1));

	rtk_pcie_16xx_ctrl_write(PCIE_CFG_CT, GO_CT);

	do {
		status = rtk_pcie_16xx_ctrl_read(PCIE_CFG_ST);
		udelay(50);
	...
	if (rtk_pcie_16xx_ctrl_read(PCIE_CFG_ST) & CFG_ST_ERROR) {
						...
						unsigned char size)
{
	...
	rtk_pcie_16xx_ctrl_write(PCIE_INDIR_CTR, 0x10);
	rtk_pcie_16xx_ctrl_write(PCIE_CFG_ST, 0x3);
	rtk_pcie_16xx_ctrl_write(PCIE_CFG_ADDR, (addr & ~0x3));
	rtk_pcie_16xx_ctrl_write(PCIE_CFG_EN,
	...
	*pdata = (rtk_pcie_16xx_ctrl_read(PCIE_CFG_RDATA) & _pci_bit_mask(mask))
							>> _pci_bit_shift(addr);
	return PCIBIOS_SUCCESSFUL;
...
static int rtk_pcie_16xx_rd_conf(struct pci_bus *bus, unsigned int devfn,
...
again:
	if (bus->number == 1 && PCI_SLOT(devfn) == 0 && PCI_FUNC(devfn) == 0) {
		if (cfg_direct_access) {
			...
			rtk_pcie_16xx_ctrl_write(0xD00, 0xFFFFF000);

			if (size == 1)
				*pval = rtk_pcie_16xx_direct_cfg_read_byte(reg);
			else if (size == 2)
				*pval = rtk_pcie_16xx_direct_cfg_read_word(reg);
			else if (size == 4)
				*pval = rtk_pcie_16xx_direct_cfg_read(reg);

			rtk_pcie_16xx_ctrl_write(0xD00, 0xFFF00000);
	...
	val = rtk_pcie_16xx_ctrl_read(0xC7C);
	if ((val & 0x1f) && retry) {
		rtk_pcie_16xx_ctrl_write(0xC7C, val);
		retry--;
		dev_err(&bus->dev, "pcie dllp error occur = 0x%x\n", val);
		goto again;
	}

	//dev_info(&bus->dev, "rtk_pcie_16xx_rd_conf reg = 0x%x, devfun:%d, size=%d, *pval = 0x%x\n",
	//							reg, devfn, size, *pval);

	return ret;
}

static int rtk_pcie_16xx_wr_conf(struct pci_bus *bus, unsigned int devfn,
			     int reg, int size, u32 val)
{
	unsigned long address;
	int ret = PCIBIOS_DEVICE_NOT_FOUND;

	//dev_info(&bus->dev, "rtk_pcie_16xx_wr_conf reg = 0x%x, val = 0x%x\n",
	//							reg, val);
	...
	if (bus->number == 1 && PCI_SLOT(devfn) == 0 && PCI_FUNC(devfn) == 0) {
		if ((reg == 0x10) || (reg == 0x18) ||
			...
			if ((val & 0x98070000) == 0x98070000 || (val & 0x98060000) == 0x98060000 ) {
				rtk_pcie_16xx_ctrl_write(0xD04,
						(val&0xfffffff0));
			}
		}
		...
		if (cfg_direct_access) {
			...
			if (size == 1)
				rtk_pcie_16xx_direct_cfg_write_byte(reg, val);
			else if (size == 2)
				rtk_pcie_16xx_direct_cfg_write_word(reg, val);
			else if (size == 4)
				rtk_pcie_16xx_direct_cfg_write(reg, val);
...
static struct pci_ops rtk_pcie_16xx_ops = {
	.read = rtk_pcie_16xx_rd_conf,
	.write = rtk_pcie_16xx_wr_conf,
};

static int rtk_pcie_16xx_hw_initial(struct device *dev)
{
	...
	do {
		pci_link_detected = (rtk_pcie_16xx_ctrl_read(0xCB4) & 0x800);
	...
	if (cfg_direct_access) {
		/* #Base 1 */
		rtk_pcie_16xx_ctrl_write(0xD08, 0x98061000);

		/* #Mask 1 */
		rtk_pcie_16xx_ctrl_write(0xD0C, 0xFFFFF000);

		/* #translate for CFG R/W */
		rtk_pcie_16xx_ctrl_write(0xD10, 0x00000000);
	}
	/* #Base 0 */
	rtk_pcie_16xx_ctrl_write(0xCFC, 0x98062000);
...
static int rtk_pcie_16xx_probe(struct platform_device *pdev)
{
	int ret = 0;
	int size = 0;
	const u32 *prop;
	...
	spin_lock_init(&rtk_pcie_16xx_lock);
...
static int rtk_pcie_16xx_suspend(struct device *dev)
{
	...
	if (RTK_PM_STATE == PM_SUSPEND_STANDBY) {
		...
		rtk_pcie_16xx_ctrl_write(0x098, 0x400);
		rtk_pcie_16xx_ctrl_write(0xC6C, 0x00000031);
...
static int rtk_pcie_16xx_resume(struct device *dev)
{
	int ret = 0;
	...
	if (RTK_PM_STATE == PM_SUSPEND_STANDBY) {
		rtk_pcie_16xx_ctrl_write(0xC6C, 0x00000032);
		dev_info(dev, "Idle mode\n");
	} else {
		dev_info(dev, "Suspend mode\n");
		if (gpio_is_valid(pcie_gpio)) {
...
static void rtk_pcie_16xx_shutdown(struct platform_device *pdev)
{
...
static const struct dev_pm_ops rtk_pcie_16xx_pm_ops = {
	...
	.resume_noirq = rtk_pcie_16xx_resume,
};
#endif //CONFIG_SUSPEND
static const struct of_device_id rtk_pcie_16xx_match_table[] = {
	{.compatible = "realtek,rtk16xx-pcie-slot1",},
	{},
};

static struct platform_driver rtk_pcie_16xx_driver = {
	.driver = {
		.name = "[RTD16xx PCIE1]",
		.of_match_table = of_match_ptr(rtk_pcie_16xx_match_table),
#ifdef CONFIG_SUSPEND
		.pm = &rtk_pcie_16xx_pm_ops,
#endif
	},
	.probe = rtk_pcie_16xx_probe,
	.shutdown = rtk_pcie_16xx_shutdown,
};
module_platform_driver(rtk_pcie_16xx_driver);
