...
/* regmap */
static const struct reg_default apw8886_reg_defaults[] = {
	{ .reg = APW8886_REG_ONOFF,        .def = 0xFD, },
	{ .reg = APW8886_REG_DISCHG,       .def = 0xFD, },
	{ .reg = APW8886_REG_DC1DC2_MODE,  .def = 0x22, },
	{ .reg = APW8886_REG_DC3_MODE,     .def = 0x20, },
	{ .reg = APW8886_REG_DC4DC5_MODE,  .def = 0x22, },
	{ .reg = APW8886_REG_LDO1_MODE,    .def = 0x20, },
	{ .reg = APW8886_REG_DC1_NRMVOLT,  .def = 0x2C, },
	{ .reg = APW8886_REG_DC2_NRMVOLT,  .def = 0x1C, },
	{ .reg = APW8886_REG_DC3_NRMVOLT,  .def = 0x24, },
	{ .reg = APW8886_REG_DC4_NRMVOLT,  .def = 0x32, },
	{ .reg = APW8886_REG_LDO1_NRMVOLT, .def = 0x12, },
	{ .reg = APW8886_REG_DC1_SLPVOLT,  .def = 0x2C, },
	{ .reg = APW8886_REG_DC2_SLPVOLT,  .def = 0x1C, },
	{ .reg = APW8886_REG_DC3_SLPVOLT,  .def = 0x24, },
	{ .reg = APW8886_REG_DC4_SLPVOLT,  .def = 0x32, },
	{ .reg = APW8886_REG_LDO1_SLPVOLT, .def = 0x12, },
	{ .reg = APW8886_REG_CLAMP,        .def = 0x00, },
};
...
static const struct regmap_config apw8886_regmap_config = {
	...
	.reg_defaults     = apw8886_reg_defaults,
	.num_reg_defaults = ARRAY_SIZE(apw8886_reg_defaults),
			     ...
			     const struct i2c_device_id *id)
{
	...
	adev->regmap = devm_rtk_regmap_init_i2c(client, &apw8886_regmap_config);
	...
	if (chip_id != 0x5a && chip_id != 0x9a) {
		dev_err(dev, "chip_id(%02x) not match\n", chip_id);
		return -EINVAL;
	}
	regmap_read(adev->regmap, APW8886_REG_VERSION, &rev);
	dev_info(dev, "apw8886(%02x) rev%d\n", chip_id, rev);

	adev->chip_id = APW888X_DEVICE_ID_APW8886;
	adev->chip_rev = rev;
	adev->dev = dev;
