...
static void _rtl8651_asicTableAccessForward(uint32 tableType, uint32 eidx, void *entryContent_P)
{
	...
	WRITE_MEM32(SWTAA, ((uint32) rtl8651_asicTableWriteAddrBase(tableType) + eidx * RTL8651_ASICTABLE_ENTRY_LENGTH));//Fill address
...
#if defined(CONFIG_RTD_1295_HWNAT)
	entryAddr = (uint32 *) (
		(uintptr_t) rtl8651_asicTableReadAddrBase(tableType) + (eidx<<5 /*RTL8651_ASICTABLE_ENTRY_LENGTH*/) ) ;
		/*(uint32) rtl8651_asicTableReadAddrBase(tableType) + eidx * RTL8651_ASICTABLE_ENTRY_LENGTH);*/
#else //defined(CONFIG_RTD_1295_HWNAT)
	entryAddr = (uint32 *) (
...
#if defined(CONFIG_RTD_1295_HWNAT)
	tmp = *(uint32 *)((uint32) rtl8651_asicTableReadAddrBase(TYPE_ACL_RULE_TABLE) + 1024 * RTL8651_ASICTABLE_ENTRY_LENGTH);
#else //defined(CONFIG_RTD_1295_HWNAT)
...
int32 _rtl8651_readAsicEntryStopTLU(uint32 tableType, uint32 eidx, void *entryContent_P)
{
...
#ifdef RTL865X_FAST_ASIC_ACCESS
	{
		...
		for( index = 0; index < _rtl8651_asicTableSize[tableType]; index++ )
		{
			...
			#if defined(CONFIG_RTD_1295_HWNAT)
			if(index<8)
				*((uint32 *)entryContent_P + index) = READ_MEM32((uintptr_t)(entryAddr + index));
			else
				*((uint32 *)entryContent_P + index) = READ_MEM32((uintptr_t)(entryAddr + index-8+8192));
			#else //defined(CONFIG_RTD_1295_HWNAT)
			if(index<8)
				*((uint32 *)entryContent_P + index) = READ_MEM32((uint32)(entryAddr + index));
			else
