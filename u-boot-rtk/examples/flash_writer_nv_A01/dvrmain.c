...
void set_spi_pin_mux(void)
{
	...
	switch (chip_id) {
		...
		default:
			...
			if (REG32(SYS_CLOCK_ENABLE5_SECURE) & 0x1) {
				// disable pcpu clk (let pcpu not access spi)
				REG32(SYS_CLOCK_ENABLE5_SECURE) = 0x2;
			}
			break;
	}
#endif
...
/*===================================thor nand dvrmain end====================================================================*/
		...
		#ifdef DUMP_NAND_FLASH
			total_block_cnt = NF_BOOTCODE_START_BLOCK + NF_BOOTCODE_BLOCK_AMOUNT;
			NF_dump_flash(device, 0x03000000, 0, total_block_cnt * pagePerBlock);
			prints("NF_dump_flash finish, use ICE to dump flash data in DDR\n");
	...
	goto finish;
		...
		spi_next_fw_start_addr += nocs_certificate_size;
		spi_scs_len += nocs_certificate_size;
		...
		spi_next_fw_start_addr += scs_params_area_size;
		// SCS data : clear copy_memory region to align 512B
		temp = scs_params_area_size % EMMC_BLOCK_SIZE;
		if( temp ) {
			set_memory((unsigned char *)DATA_TMP_ADDR, 0xFF, EMMC_BLOCK_SIZE - temp);
			if ((*do_write)(device, DATA_TMP_ADDR, (unsigned int *)spi_next_fw_start_addr, EMMC_BLOCK_SIZE-temp, 0, 0)!= 0 ) {
	        	return -5;
	    	}
			spi_next_fw_start_addr += (EMMC_BLOCK_SIZE-temp);
		}
		...
		temp = SPI_SCS_DATA_SECTION_SIZE - spi_scs_len;
		if( temp ) {
			set_memory((unsigned char *)DATA_TMP_ADDR, 0xFF, temp);
			if ((*do_write)(device, DATA_TMP_ADDR, (unsigned int *)spi_next_fw_start_addr, temp, 0, 0)!= 0 ) {
	        	return -5;
	    	}
			spi_next_fw_start_addr += temp;
		}
