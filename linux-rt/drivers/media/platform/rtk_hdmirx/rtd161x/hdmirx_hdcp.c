...
void inline Hdmi_HdcpPortCWrite(unsigned char addr, HDMI_CONST unsigned char *value, unsigned char num)
{
	...
	hdmi_rx_reg_write32(HDMI_HDCP_AP, addr, HDMI_RX_MAC);

	while (num--)
		hdmi_rx_reg_write32(HDMI_HDCP_DP, *(value++), HDMI_RX_MAC);
...
{
	hdmi_rx_reg_write32(HDMI_HDCP_AP, addr, HDMI_RX_MAC);
	return hdmi_rx_reg_read32(HDMI_HDCP_DP, HDMI_RX_MAC);
}
...
void Hdmi_Hdcp1xRepeaterAuth(void)
{
	...
	hdmi_rx_reg_mask32(HDMI_HDCP_PCR, ~(_BIT1|_BIT0), _BIT1, HDMI_RX_MAC);
	/* Set KSV list */
	...
	if (hdmi_rx_reg_read32(HDMI_HDCP_FLAG1, HDMI_RX_MAC)&HDMI_HDCP_FLAG1_wr_aksv_flag_mask) {
	...
	/* Reset AP address */
	hdmi_rx_reg_write32(HDMI_HDCP_AP, 0x43, HDMI_RX_MAC);
	...
	for (k = 0; k < CHECK_TIMES; k++) {
		...
		reg_value = hdmi_rx_reg_read32(HDMI_HDCP_FLAG1, HDMI_RX_MAC);
...
static void hdcp1x_repeater_work_fun(struct work_struct *work)
{
	...
	hdmi_rx_reg_mask32(HDMI_HDCP_FLAG2, ~HDCP_FLAG2_irq_aksv_en_mask,
		HDMI_HDCP_FLAG2_irq_aksv_en_mask, HDMI_RX_MAC);/* Enable AKSV interrupt */
}
...
void start_hdcp1x_repeater_work(void)
{
	...
	hdmirx_state.hdcp_state = HDCPRX_STATE_1PX_DISCOVER;/*  Discover HDCP 1.x */
...
void Hdmi_HdcpFSM(void)
{
	...
	hdcp_status_flag = hdmi_rx_reg_read32(HDMI_HDCP_FLAG1, HDMI_RX_MAC);

	/* Get TX write Aksv */
	if (HDMI_HDCP_FLAG1_get_wr_aksv_flag(hdcp_status_flag)) {
#ifdef CONFIG_RTK_HDCPRX_2P2
		/* Disable HDCP 2.2 */
		if (HDMI_HDCP_2P2_CR_hdcp_2p2_en(hdmi_rx_reg_read32(HDMI_HDCP_2P2_CR, HDMI_RX_MAC))) {
		...
		hdmi_rx_reg_write32(HDMI_HDCP_FLAG1, HDMI_HDCP_FLAG1_wr_aksv_flag_mask, HDMI_RX_MAC); /* Clear Aksv flag */
#endif
	}

	if (HDMI_HDCP_FLAG1_get_rd_ri_flag(hdmi_rx_reg_read32(HDMI_HDCP_FLAG1, HDMI_RX_MAC)))
		hdmi_rx_reg_write32(HDMI_HDCP_FLAG1, HDMI_HDCP_FLAG1_rd_ri_flag_mask, HDMI_RX_MAC);/* Clear Read Ri flag */

	if (hdmirx_state.hdcp_state == HDCPRX_STATE_1PX_DISCOVER) {
		/* Get Ri means HDCP 1.x nego success */
		if (HDMI_HDCP_FLAG1_get_rd_ri_flag(hdcp_status_flag))
			hdmirx_state.hdcp_state = HDCPRX_STATE_1PX_ON;
	}
}
...
void Hdmi_HdcpInit(void)
{
	...
	for (i = 0; i < 640; i++) {
		hdmi_rx_reg_write32(HDMI_HDCP_AP, 0x43 + (i<<8), HDMI_RX_MAC);
		hdmi_rx_reg_write32(HDMI_HDCP_DP, 0x00, HDMI_RX_MAC);
	}
	...
	for (i = 0; i < 320; i++)
		hdmi_rx_reg_write32(HDMI_HDCP_DKAP, rx_key.Key[i], HDMI_RX_MAC);
#endif
	...
	hdmi_rx_reg_mask32(HDMI_HDCP_FLAG2, ~HDMI_HDCP_FLAG2_irq_aksv_en_mask,
