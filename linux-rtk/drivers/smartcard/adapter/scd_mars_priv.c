...
typedef enum
{
    ...
    MAR_SCD_BAUDDIV2_UNSUPPORTED = 0xFF,
...
static MAR_SCD_CLK_LIST_t clk_info[] =
{
    [SC_CLK_9000000] = {
        .etu_list = {
            [SC_ETU_16]   = {.pre_clock_div= 0, .clock_div= 0, .baud_div1= 0, .e_baud_div2=MAR_SCD_BAUDDIV2_UNSUPPORTED},
            [SC_ETU_31]   = {.pre_clock_div= 3, .clock_div= 1, .baud_div1= 1, .e_baud_div2=MAR_SCD_BAUDDIV2_31},
            [SC_ETU_32]   = {.pre_clock_div= 3, .clock_div= 1, .baud_div1= 1, .e_baud_div2=MAR_SCD_BAUDDIV2_32},
            [SC_ETU_372]  = {.pre_clock_div= 3, .clock_div= 1, .baud_div1=12, .e_baud_div2=MAR_SCD_BAUDDIV2_31},
        },
    },
    [SC_CLK_4500000] = {
        .etu_list = {
            [SC_ETU_16]   = {.pre_clock_div= 3, .clock_div= 2, .baud_div1= 1, .e_baud_div2=MAR_SCD_BAUDDIV2_32},
            [SC_ETU_31]   = {.pre_clock_div= 6, .clock_div= 1, .baud_div1= 1, .e_baud_div2=MAR_SCD_BAUDDIV2_31},
            [SC_ETU_32]   = {.pre_clock_div= 6, .clock_div= 1, .baud_div1= 1, .e_baud_div2=MAR_SCD_BAUDDIV2_32},
            [SC_ETU_372]  = {.pre_clock_div= 6, .clock_div= 1, .baud_div1=12, .e_baud_div2=MAR_SCD_BAUDDIV2_31},
        },
    },
};
...
static int mars_scd_set_clock_by_table(mars_scd* p_this, unsigned long clk)
{
    ...
    switch(val)
    {
    case 16:    eETU = SC_ETU_16;    break;
    case 31:    eETU = SC_ETU_31;    break;
    case 32:    eETU = SC_ETU_32;    break;
    case 372:
    ...
    case 4500000:
        ...
        break;
    default:
        SC_WARNING("Unsupported sc_clk(%lu)\n", clk);
        return -1;
    }
    ...
    {
        SC_WARNING("Unsupported sc_clk for etu=%lu\n", val);
        return -1;
    }
...
static int mars_scd_set_etu_by_table(mars_scd* p_this, unsigned long etu)
{
    ...
    SC_INFO("etu(%lu), (0x%08x)->(0x%08lx), (%d, %d, %lu, %lu)\n",
        etu, GET_SCFP(p_this->base), val,
        p_this->pre_clock_div, p_this->clock_div, p_this->baud_div1, p_this->baud_div2);
...
void mars_scd_fsm_active(mars_scd* p_this)
{
    ...
    if (event & SC_RCV_INT && p_this->tx_status == SC_TX_DONE)  //RX received at least one character
   {
	...
	{
		SC_WARNING("RX Receive~~  event=%lu  \n",event);
		SET_SCCR(p_this->base, GET_SCCR(p_this->base) | SC_FIFO_RST(1));
	}
#endif
...
int mars_scd_set_clock(mars_scd* p_this, unsigned long clk)
{
...
#if defined(ENABLE_CLK_ETU_LOOKUP_TABLE)
    if( 0 == mars_scd_set_clock_by_table(p_this, clk) ) // set ok by table!!!
    {
        return 0;
    }
    else
    {
        SC_WARNING("set clock by pre_clock_div only !!!\n");
    }
#endif // ENABLE_CLK_ETU_LOOKUP_TABLE
    ...
    val |= SC_PRE_CLKDIV((p_this->pre_clock_div-1));  // oo add to sync clk config
...
#if defined(ENABLE_CLK_ETU_LOOKUP_TABLE)
    if( 0 == mars_scd_set_etu_by_table(p_this, etu) )
    {
        return 0;
    }
    else
    {
        SC_WARNING("set etu by BaudDiv1 & BaudDiv2 only !!!\n");
    }
#endif // ENABLE_CLK_ETU_LOOKUP_TABLE
...
static void mars_scd_load_gpio_config(mars_scd* p_this)
{
    ...
    p_this->pin_pwr_sel      = dts_info.pin_pwr_sel;

#if(1)
    if(p_this->vcc_5v != p_this->pwr_sel_polarity)
    {
        SC_WARNING("SC%d = default pwr changed to %s\n",
            p_this->id, p_this->pwr_sel_polarity ? "5v" : "3v");
        p_this->vcc_5v = p_this->pwr_sel_polarity;
    }
#endif
