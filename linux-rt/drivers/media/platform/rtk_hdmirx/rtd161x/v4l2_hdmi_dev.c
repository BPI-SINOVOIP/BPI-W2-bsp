...
void hdmi_buffer_finish(struct vb2_buffer *vb)
{
	if (hdmi_timestamp_mode) {
		vb->timestamp = lib_hdmi_get_clk90k();
	} else {
		/* System timer */
...
void hdmi_stop_streaming(struct vb2_queue *vq)
{
	...
	if (is_clock_enabled(CLK_MIPI) && hdmi_rx.rx_5v_state)
		stop_mipi_process();
...
void  rx5v_do_task(struct v4l2_hdmi_dev *dev)
{
	...
	state = gpio_get_value(hdmi_rx.gpio_5v_det);

	if (!state && hdmi_rx.rx_5v_state) {
		/* RX unplugged */
	...
	} else if (state && !hdmi_rx.rx_5v_state) {
		/* RX plugged */
...
void hdmi_rx_process(void)
{
	...
	switch (state) {
	case 0:
		if (hdmirx_state.DEF_ready) {
			if (hdmi_detect_mode() == _MODE_SUCCESS)
				state++;
		}
		break;
	case 1:
		ret = hdmi_check_condition_change();
		if ((ret == TRUE) && hdmirx_state.detect_done) {
			HDMIRX_INFO("Condition_change, restart HDMI Rx wrapper detection ret(%d)\n", ret);
			state = 0;
			restartHdmiRxWrapperDetection();
		} else if (hdmirx_state.detect_done == 0) {
...
static inline void update_hdmirx_switch_state(struct v4l2_hdmi_dev *dev)
{
	...
	if (hdmirx_state.measure_ready != switch_get_state(&dev->sdev)) {
		switch_set_state(&dev->sdev, hdmirx_state.measure_ready);
		pr_err("\033[0;31m[HDMI RX] switch hdmi rx state to %d\033[m\n",
			hdmirx_state.measure_ready);
	}


	if (!hdmirx_state.audio_detect_done)
		audio_count = 0;

	if (hdmirx_state.audio_detect_done != switch_get_state(&dev->asdev)) {
		if (hdmirx_state.audio_detect_done)
			audio_count++;

		if ((!hdmirx_state.audio_detect_done) || (audio_count >= 10)) {
			switch_set_state(&dev->asdev, hdmirx_state.audio_detect_done);
			HDMIRX_INFO("switch audio detect state to %d", hdmirx_state.audio_detect_done);
		}
	}

	if ((hdmirx_state.hdcp_state == HDCPRX_STATE_1PX_ON) ||
		(hdmirx_state.hdcp_state == HDCPRX_STATE_2P2_ON))
		hdcp_state = 1;
...
int hdmirx_rtk_drv_probe(struct platform_device *pdev)
{
	...
	memset(&hdmi_rx, 0, sizeof(hdmi_rx));
	memset(&hdmirx_state, 0, sizeof(hdmirx_state));
	...
	/* HPD control GPIO */
	hdmi_rx.gpio_hpd_ctrl = of_get_named_gpio(pdev->dev.of_node, "gpio-rx-hpd-ctrl", 0);
	if (hdmi_rx.gpio_hpd_ctrl < 0) {
		HDMIRX_ERROR("Get gpio-rx-hpd-ctrl fail");
		ret = -ENODEV;
		goto exit;
	} else {
		HDMIRX_INFO("HPD control gpio(%d)", hdmi_rx.gpio_hpd_ctrl);
	...
	if (hdmi_rx.gpio_5v_det < 0) {
		HDMIRX_ERROR("Get gpio-5v-detect fail");
		ret = -ENODEV;
		goto exit;
	} else {
		HDMIRX_INFO("5V detect gpio(%d)", hdmi_rx.gpio_5v_det);
		if (gpio_request(hdmi_rx.gpio_5v_det, pdev->dev.of_node->name))
			HDMIRX_ERROR("Request gpio %d fail", hdmi_rx.gpio_5v_det);

		gpio_direction_input(hdmi_rx.gpio_5v_det);
	}
