...
#ifdef CONFIG_RTK_SB2_SECURITY_DEBG
		swc_write(SWCIVA_INT | ACIVA_INT | SCIVA_INT | PCIVA_INT | WRITE_DATA_0, 0x9801A000 | 0x008);
#else
		writel( SWCIVA_INT | ACIVA_INT | SCIVA_INT | PCIVA_INT | WRITE_DATA_0, SB2_INV_INTSTAT);
#endif

		if (intr & SWCIVA_INT)
			pr_err("\033[0;31m[%s] Invalid access issued by SCPU security world\033[m\n", DRIVER_NAME) ;
		else if (intr & ACIVA_INT)
			pr_err("\033[0;31m[%s] Invalid access issued by ACPU\033[m\n", DRIVER_NAME) ;
		else if (intr & SCIVA_INT)
			pr_err("\033[0;31m[%s] Invalid access issued by SCPU\033[m\n", DRIVER_NAME) ;
		else if (intr & PCIVA_INT)
			pr_err("\033[0;31m[%s] Invalid access issued by PCPU\033[m\n", DRIVER_NAME) ;
...
static int sb2_dbg_init(struct platform_device *pdev)
{
	...
	swc_write(SB2_ACPU_INT_EN | SB2_SCPU_INT_EN | SB2_PCPU_INT_EN | WRITE_DATA_1, 0x9801A000 | 0x4E0);
	swc_write(SWCIVA_INT | ACIVA_INT | SCIVA_INT | PCIVA_INT | WRITE_DATA_0 , 0x9801A000 | 0x008);
	swc_write(SWCIVAIRQ_EN | ACIVAIRQ_EN | SCIVAIRQ_EN | PCIVAIRQ_EN | WRITE_DATA_1 | swc_read(0x9801A000 | 0x004), 0x9801A000 | 0x004);
#else
	/* Enable SB2 interrupt */
	writel(SB2_ACPU_INT_EN | SB2_SCPU_INT_EN | SB2_PCPU_INT_EN | WRITE_DATA_1, SB2_DBG_INT);
	writel(SWCIVA_INT | ACIVA_INT | SCIVA_INT | PCIVA_INT | WRITE_DATA_0 , SB2_INV_INTSTAT);
	writel(SWCIVAIRQ_EN | ACIVAIRQ_EN | SCIVAIRQ_EN | PCIVAIRQ_EN | WRITE_DATA_1 | readl(SB2_INV_INTEN), SB2_INV_INTEN);
#endif
...
#ifndef CONFIG_RTK_SB2_SECURITY_DEBG
