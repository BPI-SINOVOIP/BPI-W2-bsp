...
ifdef CONFIG_RTD1395
SUBDIR_EXAMPLES = examples/flash_writer_vm
else
SUBDIR_EXAMPLES = examples/flash_writer
endif

SUBDIR_EFUSE_EXAMPLES = examples/efuse_programmer
SUBDIR_EFUSE_VERIFY_EXAMPLES = examples/efuse_verify
.PHONY : $(SUBDIR_TOOLS) $(VERSION_FILE) $(TIMESTAMP_FILE) $(SUBDIR_EXAMPLES) $(SUBDIR_EFUSE_EXAMPLES) $(SUBDIR_EFUSE_VERIFY_EXAMPLES)
...
SUBDIRS += $(SUBDIR_EFUSE_EXAMPLES)
SUBDIRS += $(SUBDIR_EFUSE_VERIFY_EXAMPLES)
...
libs-y		:= $(patsubst %/, %/built-in.o, $(filter-out $(MIPS_BOOTLOAD_LIB_DIR)/ ,$(libs-y)))
...
ifneq ($(wildcard static_lib/libsha1_util.a),)
PLATFORM_LIBS += static_lib/libsha1_util.a
endif
ifneq ($(wildcard static_lib/libsecurity.a),)
PLATFORM_LIBS += static_lib/libsecurity.a
endif
ifneq ($(wildcard static_lib/libkeyset.a),)
PLATFORM_LIBS += static_lib/libkeyset.a
endif
...
all:		$(ALL-y)
ifndef BUILD_BOOTCODE_ONLY
	$(MAKE) -C $(SUBDIR_EXAMPLES) all PRJ=$(PRJ) CUSTOMER_ID=$(CUSTOMER_ID)\
...
u-boot.bin: u-boot FORCE
	...
	$(BOARD_SIZE_CHECK)
	@cp -v $@ $(objtree)/../rtk-pack/rtk/bpi-w2/bin/$@
...
onenand_ipl:	$(TIMESTAMP_FILE) $(VERSION_FILE) include/autoconf.mk
		$(MAKE) -C onenand_ipl/board/$(BOARDDIR) all

u-boot-onenand.bin:	onenand_ipl u-boot.bin
		cat $(ONENAND_BIN) u-boot.bin > u-boot-onenand.bin
...
#all: $(SUBDIR_EXAMPLES)
.PHONY : all
#PHONY += $(SUBDIR_EXAMPLES)
#$(SUBDIR_EXAMPLES): u-boot	$(TIMESTAMP_FILE) $(VERSION_FILE) include/autoconf.mk include/generated/generic-asm-offsets.h include/generated/asm-offsets.h
#				$(MAKE) -C $(SUBDIR_EXAMPLES) all PRJ=$(PRJ) CUSTOMER_ID=$(CUSTOMER_ID) CHIP_ID=$(CHIP_ID) CHIP_TYPE=$(CHIP_TYPE) FLASH_TYPE=$(FLASH_TYPE) CC=$(AACC)
...
clean: $(clean-dirs) clean-efuse-dir clean-flush-writer-dir
	$(call cmd,rmdirs)
	$(call cmd,rmfiles)
	@find $(if $(KBUILD_EXTMOD), $(KBUILD_EXTMOD), .) $(RCS_FIND_IGNORE) \
		...
		-o -name modules.builtin -o -name '.tmp_*.o.*' -o -name 'lib.a' \
		-o -name '*.gcno' \) -type f -print | grep -v $(SUBDIR_EFUSE_EXAMPLES) | grep -v $(SUBDIR_EFUSE_VERIFY_EXAMPLES)|xargs rm -f

clean-efuse-dir:
	$(MAKE) -C $(SUBDIR_EFUSE_EXAMPLES) clean PRJ=$(PRJ) CUSTOMER_ID=$(CUSTOMER_ID)\
	 CHIP_ID=$(CHIP_ID) CHIP_TYPE=$(CHIP_TYPE) FLASH_TYPE=$(FLASH_TYPE) CC=$(CC) LD=$(LD) OBJCOPY=$(OBJCOPY) OBJDUMP=$(OBJDUMP)
	$(MAKE) -C $(SUBDIR_EFUSE_VERIFY_EXAMPLES) clean PRJ=$(PRJ) CUSTOMER_ID=$(CUSTOMER_ID)\
	 CHIP_ID=$(CHIP_ID) CHIP_TYPE=$(CHIP_TYPE) FLASH_TYPE=$(FLASH_TYPE) CC=$(CC) LD=$(LD) OBJCOPY=$(OBJCOPY) OBJDUMP=$(OBJDUMP)

clean-flush-writer-dir:
	$(MAKE) -C $(SUBDIR_EXAMPLES) clean PRJ=$(PRJ) CUSTOMER_ID=$(CUSTOMER_ID)
