...
static int nf_erase_phys_block(n_device_type *device, UINT8 chip_sel, UINT32 page_addr)
{
	...
	if (temp == 0) {
		/* show '/' in console */
		prints("/(");
		print_hex(page_addr/pages_per_block);
		prints(")");
		return 0;
	}
	/* show 'X' in console */
...
static void NF_dump_flash_random(UINT32 dst_addr, UINT32 src_page, UINT32 page_len)
{
	...
	UINT32 spare_size = 16 * (devicetype->PageSize / 512);
	...
	print_hex(page_len);
	prints("\n");
	prints("spare size = 0x");
	print_hex(spare_size);
	prints("\n");
	...
	for (i = 0; i < page_len; i++) {
		...
		//for (j = 0; j < devicetype->PageSize / 512; j++) {
			...
			copy_memory(dst_spare_addr, REG_BASE_ADDR, spare_size);
			rtd_outl(REG_SRAM_CTL, 0x0); // disable direct access
		//}
	...
	prints("finish.\n\n");
	prints("dump nand flash summary:\n");
	prints("\taddress: ");
	print_hex(dst_addr);
	prints("\n");
	prints("\tlength: ");
	print_hex(dst_data_addr - dst_addr);
	prints("\n");
}
...
int do_identify_n(void **dev)
{
	...
	for (i = 0; i < NF_CHIP_SEL_NUM; i++)
	{
		...
		for (j = 0; j < 8; j++) {
			chip_id[j] = rtd_inb(PAGE_TMP_ADDR + j);
		}
		CHIP_ID =*(UINT32 *)chip_id;
		...
		prints("\n");
		prints("ext 4: 0x");
		print_val(chip_id[4], 2);
		prints(", ");
		prints("ext 5: 0x");
		print_val(chip_id[5], 2);
		prints(", ");
		prints("ext 6: 0x");
		print_val(chip_id[6], 2);
		prints(", ");
		prints("ext 7: 0x");
		print_val(chip_id[7], 2);
		prints("\n");
		...
		for (idx = 0; idx < DEV_SIZE_N; idx++)
		{
			...
			{
				if ((CHIP_ID & 0xffff) != n_device[idx].id)
					continue;
			}
			else if (n_device[idx].read_id_len == 4)
			{
				if (CHIP_ID != n_device[idx].id)
					continue;
			}
			else if (n_device[idx].read_id_len == 5)
			{
				...
				if ((CHIP_ID != n_device[idx].id) || (id5 != n_device[idx].id5))
					continue;
			}
			...
			*dev = (void *)&n_device[idx];
			...
			// show info
			prints("[NF] ");
			prints(n_device[idx].string);
			prints("\n");

			prints("[NF] Num of Chips: 0x");
			print_hex(n_device[idx].num_chips);
			prints("\n");

			prints("[NF] Page Size: 0x");
			print_hex(n_device[idx].PageSize);
			prints("\n");

			prints("[NF] Block Size: 0x");
			print_hex(n_device[idx].BlockSize);
			prints("\n");

			prints("[NF] Block Count(per die): 0x");
			print_hex(n_device[idx].size / n_device[idx].BlockSize);
			prints("\n");

			prints("[NF] ECC Bit: 0x");
			print_hex(n_device[idx].ecc_bit);
			prints("\n");
		...
		if (idx == DEV_SIZE_N) { // not found
			//modify by angus
			prints("Not found this nand id in list\n");
			printf("[%x %x]\n", CHIP_ID, rtd_inb(PAGE_TMP_ADDR+4));
...
int do_init_n(void *dev)
{
	...
	prints("\n");
	prints("Total Erase Block Count: 0x");
	print_hex(i);
	prints("\n");
...
void do_exit_n(void *dev)
{
	...
	hexdump("Profile:", &profile, sizeof(t_nand_profile));

	prints("Write Main Profile\n");
	res = write_nand_profile(device, &profile);
	if (res == -1) {
		prints("Write Main Profile Fail\n");
		return;
	}
	else {
		...
		if (res == -1) {
			prints("Write Backup Profile Fail\n");
			return;
		}
	}
