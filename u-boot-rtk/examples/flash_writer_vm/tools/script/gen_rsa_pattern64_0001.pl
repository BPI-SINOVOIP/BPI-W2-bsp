...
#remove first '0x' and fill 'zero'
    ...
    print("//@mod_array\n");
    if (!$no_reverse) {@np_inv32 = reverse @mod_array[0..7];}
    else { my $end = $KEY_BITS/8 -1;
        @np_inv32 = @mod_array[$end-7..$end];}
    ...
    if($word_swap)
    {
        ...
        $tmp = $np_inv32[$i*4+4];
        $np_inv32[$i*4+4] = $np_inv32[$i*4+7];
        $np_inv32[$i*4+7] = $tmp;
        
        $tmp = $np_inv32[$i*4+5];
        $np_inv32[$i*4+5] = $np_inv32[$i*4+6];
        $np_inv32[$i*4+6] = $tmp;
    }
...
if(@key_files > 1)
{
    ...
    print ("unsigned long long np_inv32[$key_count] = {");
    print (join( ', ', @np_inv32_total));
    print ("\n};\n");
}
else
{
    print "unsigned long long np_inv32 = $np_inv;\n";
...
sub r_power
{
    return (($KEY_BITS/64)+2)*64*2; #0001 ECO, hwrsa r_sel=1
}
