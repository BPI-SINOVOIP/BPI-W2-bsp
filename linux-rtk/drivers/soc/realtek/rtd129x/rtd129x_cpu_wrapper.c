...
irqreturn_t scpu_wrapper_isr(int irq, void *reg_base)
{
	...
	pr_err("[%s] scpu wrapper get int 0x%08x", DEV_NAME, intr);
	if (intr & (BIT(4) | BIT(2))) {
		writel((intr & ~(BIT(3)|BIT(1))), scpu_wrap_addr + DBG_INT);
		pr_err("[%s] scpu addr:0x%08x mode:%s",
			DEV_NAME, addr, (cause & 1) ? "W" : "R");

		sprintf(buf, "[%s] Memory 0x%08x trashed with %s\n",
			DEV_NAME, addr, (cause & 1) ? "W" : "R");
		die(buf, regs, 0);
...
static int __init scpu_wrapper_init(void)
{
	...
	pr_info("[%s] scpu wrappee irq %d, scpu_wrap_addr 0x%08llx\n",
		DEV_NAME, irq, (u64)scpu_wrap_addr);

	of_node_put(np);

	results = request_irq(irq,
		scpu_wrapper_isr,
		IRQF_SHARED,
		"scpu_wrapper",
		scpu_wrap_addr);
