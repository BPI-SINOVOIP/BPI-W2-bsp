...
static int32 rtl865x_nat_callbackFn_for_del_ip(void *param)
{
	...
	for (i = 0; i < RTL8651_TCPUDPTBL_SIZE; i++) {
		if ((nat_tbl.nat_bucket[i].flags & NAT_OUTBOUND) &&
			(nat_tbl.nat_bucket[i].ext_ip_ == natip->extIpAddr)) {
			memcpy(&nat_tuple, &nat_tbl.nat_bucket[i].tuple_info,
				sizeof(struct nat_tuple));
			nat_out = &nat_tbl.nat_bucket[i];
			nat_in = &nat_tbl.nat_bucket[nat_out->in];
			rtl8651_delAsicNaptTcpUdpTable(nat_out->in,
				nat_out->in);
			rtl8651_delAsicNaptTcpUdpTable(nat_out->out,
				nat_out->out);
...
int32 rtl_nat_expire_interval_update(int proto, int32 interval)
{
	...
	asic_interval = interval > RTL_NAT_AVG_INTERVAL ?
		interval - RTL_NAT_AVG_INTERVAL :
		interval - RTL_NAT_MIN_INTERVAL;
	asic_interval = asic_interval < RTL_NAT_MIN_INTERVAL ?
		RTL_NAT_MIN_INTERVAL : asic_interval;
...
static struct nat_entry *_rtl865x_nat_outbound_lookup(struct nat_tuple
	*nat_tuple)
{
	struct nat_entry *nat_out;
	uint32 i, hash;

	hash = rtl8651_naptTcpUdpTableIndex((uint8)nat_tuple->proto,
		nat_tuple->int_host.ip, nat_tuple->int_host.port,
...
static struct nat_entry *_rtl865x_nat_inbound_lookup(struct nat_tuple
	*nat_tuple)
{
...
static int _rtl865x_isEntryPreReserved(uint32 index)
{
	...
	if ((natEntry->flags & NAT_PRE_RESERVED)) {
		if (jiffies >= natEntry->reserveTime) {
			if (jiffies >
				(natEntry->reserveTime +
					RESERVE_EXPIRE_TIME * HZ)) {
			...
			/*timer overflow */
			if (((0xFFFFFFFF - natEntry->reserveTime) + jiffies + 1) >
				(RESERVE_EXPIRE_TIME * HZ)) {
				natEntry->flags &= (~(NAT_PRE_RESERVED));
				natEntry->reserveTime = 0;
				return FALSE;
	...
	rtl865x_naptHashInfo_t * naptHashInfo)
{
	...
	in = rtl8651_naptTcpUdpTableIndex((uint8)(naptEntry->protocol),
		naptEntry->remIp, naptEntry->remPort, naptEntry->extIp,
		naptEntry->extPort);
	out = rtl8651_naptTcpUdpTableIndex((uint8)(naptEntry->protocol),
		naptEntry->intIp, naptEntry->intPort, naptEntry->remIp,
		naptEntry->remPort);
	...
	if (rtl865x_enableNaptFourWay == TRUE) {
		...
		for (i = 0; i < 4; i++) {
			natEntry = &nat_tbl.nat_bucket[index];
			if (NAT_INUSE(natEntry) ||
				_rtl865x_isEntryPreReserved(index)) {
...
int rtl865x_optimizeExtPort(unsigned short origDelta, unsigned int rangeSize,
	unsigned short *newDelta)
{
	int i;
	int msb;
...
static int32 _rtl865x_addNaptConnection(rtl865x_napt_entry *naptEntry,
	rtl865x_priority * prio)
{
	int32 retval;
	...
	very = rtl8651_naptTcpUdpTableIndex(((uint8)naptEntry->protocol) |
		HASH_FOR_VERI, htonl(naptEntry->remIp),
		htons((uint16)(naptEntry->remPort)), 0, 0);
	in = rtl8651_naptTcpUdpTableIndex((uint8)naptEntry->protocol,
		htonl(naptEntry->remIp), htons((uint16)(naptEntry->remPort)),
		htonl(naptEntry->extIp), extPort);
	out = rtl8651_naptTcpUdpTableIndex((uint8)naptEntry->protocol,
	...
	if (NAT_INUSE(nat_out)) {
		...
		if (((nat_out->tuple_info.proto == RTL865X_PROTOCOL_UDP) &&
			(elasped > UDP_OVERRIDE_ELASPED_THRESHOLD)) ||
			((nat_out->tuple_info.proto == RTL865X_PROTOCOL_TCP) &&
			(elasped > TCP_OVERRIDE_ELASPED_THRESHOLD))) {
	...
	rtlglue_printf("LR(%s):  %s (%u.%u.%u.%u:%u -> %u.%u.%u.%u:%u) g:(%u.%u.%u.%u:%u)\n",
		...
		NIPQUAD(naptEntry->intIp), ntohs(naptEntry->intPort),
		NIPQUAD(naptEntry->remIp), ntohs(naptEntry->remPort),
		NIPQUAD(naptEntry->extIp), ntohs(naptEntry->extPort));
#endif
	...
	for (i = 0; i < 2; i++) {
		if (rtl865x_getRouteEntry(sip, &rt) == SUCCESS) {
			...
			rule4[i].ruleType_ = (naptEntry->protocol == RTL865X_PROTOCOL_TCP ?
				RTL865X_ACL_TCP : RTL865X_ACL_UDP);
			rule4[i].srcIpAddr_ = sip;
			rule4[i].dstIpAddr_ = dip;
			rule4[i].tcpSrcPortLB_ = sport;
			rule4[i].tcpDstPortLB_ = dport;
			rule4[i].netifIdx_ = _rtl865x_getNetifIdxByNameExt(
							rt.dstNetif->name);
...

	{
		...
		for (i = 0; i < 2; i++) {
			if (rtl865x_getArpMapping(sip, &arpInfo) == SUCCESS &&
				rtl865x_getRouteEntry(sip, &rt) == SUCCESS) {
				/*      check mac base rule secondly    */
				//memset(&rule2[i], 0, sizeof(rtl865x_AclRule_t));
				rule2[i].ruleType_ = RTL865X_ACL_MAC;
				memcpy(rule2[i].srcMac_.octet,
					arpInfo.mac.octet, ETHER_ADDR_LEN);
				memset(rule2[i].srcMacMask_.octet, 0xff,
					ETHER_ADDR_LEN);
				rule2[i].netifIdx_ = _rtl865x_getNetifIdxByNameExt(
								rt.dstNetif->name);
				...
				if (rtl865x_qosCheckNaptPriority(&rule2[i]) ==
					SUCCESS) {
		...
		for (i = 0; i < 2; i++) {
			if ((rule2[i].aclIdx == 0) &&
				rtl865x_getArpMapping(dip, &arpInfo) == SUCCESS &&
				rtl865x_getRouteEntry(naptEntry->remIp, &rt) == SUCCESS) {
				/*      check mac base rule secondly  */
				//memset(&rule2[i], 0, sizeof(rtl865x_AclRule_t));
				rule2[i].ruleType_ = RTL865X_ACL_MAC;
				memcpy(rule2[i].dstMac_.octet,
					arpInfo.mac.octet, ETHER_ADDR_LEN);
				memset(rule2[i].dstMacMask_.octet, 0xff,
					ETHER_ADDR_LEN);
				rule2[i].netifIdx_ = _rtl865x_getNetifIdxByNameExt(
					rt.dstNetif->name);
	...
	for (i = 0; i < 2; i++) {
		if (rule4[i].aclIdx && rule2[i].aclIdx) {
			priority[i] = (rule4[i].aclIdx < rule2[i].aclIdx) ?
				rule4[i].priority_ : rule2[i].priority_;
			upDown[i] = (rule4[i].aclIdx < rule2[i].aclIdx) ?
				rule4[i].upDown_ : rule2[i].upDown_;
	...
	for (i = 0; i < 2; i++) {
		...
		asic_nat.isStatic = 1;
		asic_nat.isTcp = (naptEntry->protocol == RTL865X_PROTOCOL_TCP) ?
									1 : 0;
		asic_nat.isValid = 1;
		asic_nat.offset = ((i == 0) ? offset : (extPort & 0x3f));
		asic_nat.selEIdx = ((i == 0) ? selEidx_out : very & 0x3ff);
		asic_nat.selExtIPIdx = ((i == 0) ? ipidx :
						((extPort & 0x3ff) >> 6));
		...
		asic_nat.ageSec = (naptEntry->protocol == RTL865X_PROTOCOL_TCP) ?
			nat_tbl.tcp_timeout : nat_tbl.udp_timeout;
#if	defined(CONFIG_RTL_HW_QOS_SUPPORT)
		...
		rtl8651_setAsicNaptTcpUdpTable(1, ((i == 0) ? out : in),
			&asic_nat);
	}
...
static int32 _rtl865x_naptIdleCheck(int32 index)
{
	...
	if (entry.isTcp) {
		return (nat_tbl.tcp_timeout > hw_now ?
			nat_tbl.tcp_timeout - hw_now : 0);
	} else {
		return (nat_tbl.udp_timeout > hw_now ?
			nat_tbl.udp_timeout - hw_now : 0);
	}
}
...
static int32 _rtl865x_naptSync(rtl865x_napt_entry *naptEntry, uint32 refresh)
{
	...
	if ((nat_out != NULL) && (nat_in != NULL)) {
		rc = rtl8651_getAsicNaptTcpUdpTable(nat_out->out,
			&asic_nat_out);
		assert(rc == SUCCESS);
		...
		hw_now = (asic_nat_out.ageSec >
			asic_nat_in.ageSec) ? asic_nat_out.ageSec : asic_nat_in.
			ageSec;
	...
	if (hw_now == 0) {
		...
		if (naptEntry->protocol == RTL865X_PROTOCOL_UDP) {
			return (nat_tbl.udp_timeout >
				hw_now ? nat_tbl.udp_timeout - hw_now : 0);
		} else if (naptEntry->protocol == RTL865X_PROTOCOL_TCP) {
			return (nat_tbl.tcp_timeout >
				hw_now ? nat_tbl.tcp_timeout - hw_now : 0);
		}
	}
...
*/
int32 rtl865x_addNaptConnection(rtl865x_napt_entry *naptEntry,
	rtl865x_priority * prio)
{
	int32 retval = FAILED;
...
static int32 rtl865x_nat_callbackFn_for_del_arp(void *param)
{
	...
	for (i = 0; i < RTL8651_TCPUDPTBL_SIZE; i++) {
		...
		if (NAT_INUSE(natEntryPtr)) {
			...
			if ((arpEntry->ip == natEntryPtr->int_ip_) ||
				(arpEntry->ip == natEntryPtr->rem_ip_)) {
#if 0
...
inline static int32 rtl865x_naptSetAsicWithPriority(struct nat_entry *entry,
	int32 priority)
{
	rtl865x_tblAsicDrv_naptTcpUdpParam_t asic_nat;
	int32 idx;
		...
		if (NAT_INUSE(&nat_tbl.nat_bucket[i])) {
			...
			if (nat_this->flags & NAT_PRI_PROCESSED) {
				...
				if (rtl865x_getRouteEntry(sip, &rt) == SUCCESS) {
					...
					rule4.ruleType_ = (nat_this->proto_ == RTL865X_PROTOCOL_TCP ?
						RTL865X_ACL_TCP : RTL865X_ACL_UDP);
					...
					if (rule4.netifIdx_ < 0 ||
						rule4.netifIdx_ >= NETIF_NUMBER) {
					...
					if (rtl865x_getArpMapping(sip,
							&arpInfo) == SUCCESS) {
						memset(&rule2, 0,
							sizeof(rtl865x_AclRule_t));
						rule2.ruleType_ = RTL865X_ACL_MAC;
						memcpy(rule2.srcMac_.octet,
							arpInfo.mac.octet,
							ETHER_ADDR_LEN);
						memset(rule2.srcMacMask_.octet,
							0xff, ETHER_ADDR_LEN);
					...
					if (rule4.aclIdx && rule2.aclIdx) {
						priority = (rule4.aclIdx < rule2.aclIdx) ?
							rule4.priority_ : rule2.priority_;
					...
					if (priority > -1) {
						...
						if (nat_that != NULL) {
							rtl865x_naptSetAsicWithPriority(nat_that,
								priority);
						}

						if (nat_this->flags & NAT_PRI_HALF_PROCESSED)
						{
							CLR_NAT_FLAGS(nat_this,
								NAT_PRI_HALF_PROCESSED);
							num++;
						} else {
							if (nat_that != NULL) {
								SET_NAT_FLAGS(nat_that,
									NAT_PRI_PROCESSED);
							} else {
						...
						if (nat_this->flags & NAT_PRI_HALF_PROCESSED)
						{
							rtl865x_naptSetAsicWithPriority(nat_this,
								defPriority > -1 ?
								defPriority : 0);
		...
		if (NAT_INUSE(tmp)) {
			...
			if (nat_out == nat_in) {
				if (nat_out->flags & NAT_OUTBOUND) {
					rtl8651_delAsicNaptTcpUdpTable(outIndex,
						outIndex);
					memset(nat_out, 0, sizeof(*nat_out));
					nat_tbl.freeEntryNum++;
				} else if (nat_in->flags & NAT_INBOUND) {
					rtl8651_delAsicNaptTcpUdpTable(inIndex,
						inIndex);
					memset(nat_in, 0, sizeof(*nat_in));
					nat_tbl.freeEntryNum++;
				} else {
...
#if defined(CONFIG_RTL_EXT_PORT_SUPPORT)
				seq_printf(s,
					"[%4d]%s:%d.%d.%d.%d:%d---->%d.%d.%d.%d:%d---->%d.%d.%d.%d:%d\n	  flags:0x%x,outbound:(%d),inbound:(%d) ,isExtNaptEntry(%d)\n",
#else
				seq_printf(s,
					"[%4d]%s:%d.%d.%d.%d:%d---->%d.%d.%d.%d:%d---->%d.%d.%d.%d:%d\n      flags:0x%x,outbound:(%d),inbound:(%d)\n",
#endif
					i,
					natEntryPtr->proto_ == 1 ? "tcp" : "udp",
					NIPQUAD(natEntryPtr->int_ip_),
					natEntryPtr->int_port_,
					NIPQUAD(natEntryPtr->ext_ip_),
					natEntryPtr->ext_port_,
					NIPQUAD(natEntryPtr->rem_ip_),
					natEntryPtr->rem_port_,
					natEntryPtr->flags, natEntryPtr->out,
					natEntryPtr->in
#if defined(CONFIG_RTL_EXT_PORT_SUPPORT)
...
int32 rtl865x_sw_napt_seq_write(struct file *filp, const char *buff,
	...
	if (buff && !copy_from_user(tmpbuf, buff, len)) {
		...
		if (!memcmp(cmd_addr, "flush", 5)) {
			...
			if (NAT_INUSE(tmp)) {
				...
				if ((nat_out == NULL) && (nat_in == NULL)) {
					rtl8651_delAsicNaptTcpUdpTable(delIndex,
						delIndex);
					goto errout;
				}
...
#if defined(CONFIG_RTL_EXT_PORT_SUPPORT)
int rtl_check_ext_port_napt_entry(uint32 sip, uint16 sport, uint32 dip,
	uint16 dport, uint8 protocol)
{
	...
	nat_tuple = &temp;
	nat_tuple->proto = ((protocol == 6) ? RTL865X_PROTOCOL_TCP :
					      RTL865X_PROTOCOL_UDP);
	...
	//outbound
	hash = rtl8651_naptTcpUdpTableIndex((uint8)nat_tuple->proto,
		htonl(nat_tuple->int_host.ip), htons(nat_tuple->int_host.port),
		htonl(nat_tuple->rem_host.ip), htons(nat_tuple->rem_host.port));
	//panic_printk("hash:%d,[%s]:[%d].\n",hash,__FUNCTION__,__LINE__);
	if (rtl865x_enableNaptFourWay == TRUE) {
		for (i = 0; i < 4; i++) {
			nat_out = &nat_tbl.nat_bucket[hash];
			if (((htonl(sip) == nat_out->int_ip_) &&
					(htons(sport) == nat_out->int_port_) &&
					(htonl(dip) == nat_out->rem_ip_) &&
					(htons(dport) == nat_out->rem_port_) &&
					(nat_tuple->proto == nat_out->proto_) &&
					(nat_out->flags & NAT_OUTBOUND))
				) {
				return 1;
			}
...
int32 rtl865x_flush_ext_port_entry(void)
{
	...
	for (i = 0; i < RTL8651_TCPUDPTBL_SIZE; i++) {
		...
		if (NAT_INUSE(tmp)) {
			...
			if ((nat_out->isExtNaptEntry) && (nat_out == NULL) &&
				(nat_in->isExtNaptEntry) && (nat_in == NULL)) {
				rtl8651_delAsicNaptTcpUdpTable(outIndex,
					inIndex);
				continue;
			}
...
int rtl865x_check_hw_nat_by_ip(ipaddr_t ip)
{
	...
	for (i = 0; i < RTL8651_TCPUDPTBL_SIZE; i++) {
		natEntryPtr = &nat_tbl.nat_bucket[i];
		if (NAT_INUSE(natEntryPtr)) {
			if ((ip == natEntryPtr->int_ip_) ||
				(ip == natEntryPtr->rem_ip_)) {
				ret = SUCCESS;
				break;
			}
		}
