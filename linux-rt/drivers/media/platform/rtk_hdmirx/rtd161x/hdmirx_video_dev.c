...
static long v4l2_hdmi_do_ioctl(struct file *file, unsigned int cmd, void *arg)
{
	...
	case VIDIOC_ENUM_FRAMESIZES:
	{
		...
		if (((hdmirx_state.measure_ready == 0) && (mipi_top.src_sel == 1)) ||
			(fsize->index != 0)) {
			ret = -EINVAL;
			goto exit;
		}
