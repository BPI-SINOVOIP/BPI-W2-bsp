...
int tar_check_header(	char *cheader ) {
	...
	if (tar_checksum((char *)header) != simple_strtol(header->chksum, 0, 8))  {
		// true tar function
    ...
    char *dst_addr, int* length, char *name)
{
    ...
    for (;index >= 0;)
    {
	 ...
	 data = &src_addr[pos + 512];
        file_size = simple_strtol(header->size ,0,8);
    ...
    unsigned int *dst_length )
{
	...
	for (;;)
	{
		...
		if (!strncmp(s,header,100))
		{
			...
			if (sum != simple_strtol(header + 148,0,8))
			{
			...
			TAR_DEBUG("dump 0x%x 0x%x", header + 512, simple_strtol(header + 124,0,8));

			*dst_addr = header + 512;
			*dst_length = simple_strtol(header + 124,0,8);

			return fd;
		}

		len = simple_strtol(header + 124,0,8);
