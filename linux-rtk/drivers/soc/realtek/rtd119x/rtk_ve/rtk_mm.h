...
typedef unsigned long long  mem_key_t;

#define MEM_PAGE_SIZE           (16*1024)
#define MAKE_KEY(_a, _b)        (((mem_key_t)_a)<<32 | _b)
    ...
    )
{
    ...
    rtk_avltree_free(tree->left);
    tree->left = NULL;
    rtk_avltree_free(tree->right);
    tree->right = NULL;
}
    ...
    )
{
    ...
    mm->base_addr  = (addr+(MEM_PAGE_SIZE-1))&~(MEM_PAGE_SIZE-1);
    mm->mem_size   = size&~MEM_PAGE_SIZE;
    mm->num_pages  = mm->mem_size/MEM_PAGE_SIZE;
    ...
    for (i=0; i<mm->num_pages; i++) {
        ...
        mm->page_list[i].addr         = mm->base_addr + i*MEM_PAGE_SIZE;
        mm->page_list[i].alloc_pages  = 0;
        mm->page_list[i].used         = 0;
        mm->page_list[i].first_pageno = -1;
    }
    ...
    )
{
    ...
    if (mm->free_tree) {
        rtk_avltree_free(mm->free_tree);
    }
    if (mm->alloc_tree) {
        rtk_avltree_free(mm->alloc_tree);
    }
    ...
    )
{
	...
	npages = (size + MEM_PAGE_SIZE -1)/MEM_PAGE_SIZE;
