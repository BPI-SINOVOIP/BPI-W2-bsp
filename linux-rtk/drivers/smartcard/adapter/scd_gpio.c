...
{    
    ...
    p_scd = gpio_scd_open((dev->id & 0x3));
...
{       
    gpio_scd_close((gpio_scd*) scd_get_drvdata(dev));
}
...
{    
    gpio_scd* p_this = (gpio_scd*) scd_get_drvdata(dev);
    return gpio_scd_enable(p_this, on_off);        
}
...
{    
    ...
    case SC_PARAM_VCC_LVL:
        ret = gpio_scd_set_vcc_lvl(p_this, (SC_VCC_LVL) val);   
        break;
        
    case SC_PARAM_CLOCK:
        ret = gpio_scd_set_clock(p_this, (unsigned long) val);        
        break;
        
    case SC_PARAM_ETU:
        ret = gpio_scd_set_etu(p_this, (unsigned long) val); 
        break;
        
    case SC_PARAM_CONV:
        ret = gpio_scd_set_convention(p_this, (SC_CONV) val);   
        break;
        
    case SC_PARAM_PARITY:
        ret = gpio_scd_set_parity(p_this, (SC_PARITY) val);   
        break;
...
{    
    ...
    {        
    case SC_PARAM_VCC_LVL:
        ret = gpio_scd_get_vcc_lvl(p_this, (SC_VCC_LVL*) p_val);   
        break;
        
    case SC_PARAM_CLOCK:        
        ret = gpio_scd_get_clock(p_this,  p_val);        
        break;
        
    case SC_PARAM_ETU:
        ret = gpio_scd_get_etu(p_this, p_val); 
        break;
        
    case SC_PARAM_CONV:
        ret = gpio_scd_get_convention(p_this, (SC_CONV*) p_val);   
        break;
        
    case SC_PARAM_PARITY:
        ret = gpio_scd_get_parity(p_this, (SC_PARITY*) p_val);   
        break;
...
{            
    gpio_scd* p_this = (gpio_scd*) scd_get_drvdata(dev);    
    return gpio_scd_activate(p_this);    
}
...
{            
    gpio_scd* p_this = (gpio_scd*) scd_get_drvdata(dev);    
    return gpio_scd_deactivate(p_this);    
}
...
{
    gpio_scd* p_this = (gpio_scd*) scd_get_drvdata(dev);              
    return gpio_scd_get_atr(p_this, p_atr);            
}
...
{
    gpio_scd* p_this = (gpio_scd*) scd_get_drvdata(dev);    
    return gpio_scd_poll_card_status(p_this);    
}
...
{   
    gpio_scd* p_this = (gpio_scd*) scd_get_drvdata(dev);    
    return gpio_scd_xmit(p_this, p_data, len);     
}                                 
...
{   
    gpio_scd* p_this = (gpio_scd*) scd_get_drvdata(dev);
    return gpio_scd_read(p_this, p_data, len);
}                                 
                                                                  

static scd_device gpio_scd_controller[1] = 
{
    {
        .id   = ID_MARS_SCD(0),
        .name = "gpio_scd_0"
...
static scd_driver gpio_scd_driver = 
{    
    .name               = "gpio_scd",
    .probe              = ops_probe,
    .remove             = ops_remove,        
...
/*------------------------------------------------------------------
 * Func : gpio_scd_module_init
 *
 * Desc : init gpio smart card interface driver
 *
 * Parm : N/A
 *         
...
{                        
    ...
    register_scd_device(&gpio_scd_controller[0]);          // register scd device
...
/*------------------------------------------------------------------
 * Func : gpio_scd_module_exit
 *
 * Desc : uninit gpio smart card interface driver
 *
 * Parm : N/A
 *         
 * Retn : 0 : success, others fail  
 *------------------------------------------------------------------*/
static void __exit gpio_scd_module_exit(void)
{            
    unregister_scd_device(&gpio_scd_controller[0]);    
    unregister_scd_driver(&gpio_scd_driver);
}


module_init(gpio_scd_module_init);
module_exit(gpio_scd_module_exit);
