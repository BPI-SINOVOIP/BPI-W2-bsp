					...
					void *buffer)
{
	...
	blks = part->mmc_blk->block_read(part->mmc_blk->dev,
			       ...
			       lbaint_t sectors, void *buffer)
{
	...
	return part->mmc_blk->block_write(part->mmc_blk->dev,
			  start, sectors, tmp_buf);
}
...
static struct mmc_part *get_partition(AvbOps *ops, const char *partition)
{
	...
	ret = mmc_switch_part(dev_num, part_num);
	if (ret)
		goto err;

	mmc_blk = &(part->mmc->block_dev);
	if (!mmc_blk) {
		printf("Error - failed to obtain block descriptor\n");
		goto err;
	}

	ret = get_partition_info_efi_by_name(mmc_blk, partition, &part->info);
	if (ret) {
...
{
	printf("validate_vbmeta_public_key...\n");
	if (avb_memcmp((void *)public_key_data, (const void *)pkmd, public_key_length) != 0) {
		printf("validate_vbmeta_public_key FAIL \n");
...
static AvbIOResult read_is_device_unlocked(AvbOps *ops, bool *out_is_unlocked)
{
	...
	printf("read_is_device_unlocked...\n");
