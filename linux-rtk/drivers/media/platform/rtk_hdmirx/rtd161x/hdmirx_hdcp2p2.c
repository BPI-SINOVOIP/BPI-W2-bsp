...
char Get_Hdmi_hdcp_2_2_Write_Message(void)
{
	...
	wr_msg_done = HDMI_HDCP_2P2_SR0_get_irq_wr_msg_done(hdmi_rx_reg_read32(HDMI_HDCP_2P2_SR0, HDMI_RX_MAC));
	if (wr_msg_done)
		return 1;
	else
		return 0;
...
char Get_Hdmi_hdcp_2_2_Read_Message(void)
{
	...
	rd_msg_done = HDMI_HDCP_2P2_SR0_get_irq_rd_msg_done(hdmi_rx_reg_read32(HDMI_HDCP_2P2_SR0, HDMI_RX_MAC));
	if (rd_msg_done)
		return 1;
	else
		return 0;
...
{
	hdmi_rx_reg_write32(HDMI_HDCP_2P2_SR0, HDMI_HDCP_2P2_SR0_irq_rd_msg_done_mask, HDMI_RX_MAC);
}

void HW_Cipher_Setting(unsigned char *lc128)
{
	hdmi_rx_reg_mask32(HDMI_HDCP_2P2_CR, ~(HDMI_HDCP_2P2_CR_fw_mode_riv_mask),
		HDMI_HDCP_2P2_CR_fw_mode_riv_mask, HDMI_RX_MAC);

	/* lc 128 */
	hdmi_rx_reg_mask32(HDMI_HDCP_2P2_LC0, ~HDMI_HDCP_2P2_LC0_lc_mask,
		(lc128[15]|(lc128[14]<<8)|(lc128[13]<<16)|(lc128[12]<<24)), HDMI_RX_MAC);
	hdmi_rx_reg_mask32(HDMI_HDCP_2P2_LC1, ~HDMI_HDCP_2P2_LC1_lc_mask,
		(lc128[11]|(lc128[10]<<8)|(lc128[9]<<16)|(lc128[8]<<24)), HDMI_RX_MAC);
	hdmi_rx_reg_mask32(HDMI_HDCP_2P2_LC2, ~HDMI_HDCP_2P2_LC2_lc_mask,
		(lc128[7]|(lc128[6]<<8)|(lc128[5]<<16)|(lc128[4]<<24)), HDMI_RX_MAC);
	hdmi_rx_reg_mask32(HDMI_HDCP_2P2_LC3, ~HDMI_HDCP_2P2_LC3_lc_mask,
...
void Set_Rx_status_Ready(void)
{
	...
	hdmi_rx_reg_write32(HDMI_HDCP_AP, 0x71, HDMI_RX_MAC);
	hdmi_rx_reg_write32(HDMI_HDCP_DP, (0x1 << 2), HDMI_RX_MAC);
}

void DownStream_done(void)
{
	hdmi_rx_reg_mask32(HDMI_HDCP_2P2_CR, ~HDMI_HDCP_2P2_CR_downstream_done_mask,
		HDMI_HDCP_2P2_CR_downstream_done_mask, HDMI_RX_MAC);
}
void KS_Setting(unsigned char *bKs)
{
	hdmi_rx_reg_mask32(HDMI_HDCP_2P2_CR,
		~(HDMI_HDCP_2P2_CR_fw_mode_riv_mask), HDMI_HDCP_2P2_CR_fw_mode_riv_mask, HDMI_RX_MAC);
	/* Initial ks value */
	hdmi_rx_reg_mask32(HDMI_HDCP_2P2_KS0, ~HDMI_HDCP_2P2_KS0_ks_mask,
		((*(bKs+15))|((*(bKs+14))<<8)|((*(bKs+13))<<16)|((*(bKs+12))<<24)), HDMI_RX_MAC);
	hdmi_rx_reg_mask32(HDMI_HDCP_2P2_KS1, ~HDMI_HDCP_2P2_KS1_ks_mask,
		((*(bKs+11))|((*(bKs+10))<<8)|((*(bKs+9))<<16)|((*(bKs+8))<<24)), HDMI_RX_MAC);
	hdmi_rx_reg_mask32(HDMI_HDCP_2P2_KS2, ~HDMI_HDCP_2P2_KS2_ks_mask,
		((*(bKs+7))|((*(bKs+6))<<8)|((*(bKs+5))<<16)|((*(bKs+4))<<24)), HDMI_RX_MAC);
	hdmi_rx_reg_mask32(HDMI_HDCP_2P2_KS3, ~HDMI_HDCP_2P2_KS3_ks_mask,
		((*(bKs+3))|((*(bKs+2))<<8)|((*(bKs+1))<<16)|((*bKs)<<24)), HDMI_RX_MAC);

	hdmi_rx_reg_mask32(HDMI_HDCP_2P2_CR, ~HDMI_HDCP_2P2_CR_ks_done_mask,
		HDMI_HDCP_2P2_CR_ks_done_mask, HDMI_RX_MAC);
...
{
	hdmi_rx_reg_mask32(HDMI_HDCP_2P2_RIV0, ~HDMI_HDCP_2P2_RIV0_riv_mask,
		(*(bRiv+7)|(*(bRiv+6)<<8)|(*(bRiv+5)<<16)|(*(bRiv+4)<<24)), HDMI_RX_MAC);
	hdmi_rx_reg_mask32(HDMI_HDCP_2P2_RIV1, ~HDMI_HDCP_2P2_RIV1_riv_mask,
	...
	hdmirx_state.hdcp_state = HDCPRX_STATE_2P2_ON;/* HDCP 2.2 nego success */
}
...
void Hdmi_HDCP_2_2_Init(void)
{
	...
	hdmi_rx_reg_mask32(HDMI_HDCP_2P2_CR, ~HDMI_HDCP_2P2_CR_hdcp_2p2_en_mask,
		0, HDMI_RX_MAC);
	hdmi_rx_reg_mask32(HDMI_HDCP_2P2_SR1, ~HDMI_HDCP_2P2_SR1_irq_wr_msg_done_en_mask,
		HDMI_HDCP_2P2_SR1_irq_wr_msg_done_en_mask, HDMI_RX_MAC);
...
char  Hdmi_HDCP2_2_Write_Data_to_TX(unsigned char *bSendData, unsigned int wLen)
{
	...
	for (i = 0; i < wLen; i++) {
		hdmi_rx_reg_write32(HDMI_HDCP_MSAP, i, HDMI_RX_MAC);
		hdmi_rx_reg_write32(HDMI_HDCP_MSDP, *(bSendData+i), HDMI_RX_MAC);
	}

	hdmi_rx_reg_write32(HDMI_HDCP_AP, HDCP_RXstatus_addr, HDMI_RX_MAC);
	hdmi_rx_reg_write32(HDMI_HDCP_DP, (wLen&0xFF), HDMI_RX_MAC);
	hdmi_rx_reg_write32(HDMI_HDCP_AP, HDCP_RXstatus_addr+1, HDMI_RX_MAC);
	hdmi_rx_reg_write32(HDMI_HDCP_DP, ((wLen&0x3FF)>>8), HDMI_RX_MAC);
...
{
	hdmi_rx_reg_mask32(HDMI_HDCP_2P2_CR, ~(HDMI_HDCP_2P2_CR_switch_state_mask),
		HDMI_HDCP_2P2_CR_switch_state(0x10), HDMI_RX_MAC);/* set authenticated */
	hdmi_rx_reg_mask32(HDMI_HDCP_2P2_CR, ~(HDMI_HDCP_2P2_CR_apply_state_mask | HDMI_HDCP_2P2_CR_fw_mode_riv_mask),
		(HDMI_HDCP_2P2_CR_apply_state_mask | HDMI_HDCP_2P2_CR_fw_mode_riv_mask), HDMI_RX_MAC);
...
void  Hdmi_HDCP_2_2_msg_hander(void)
{
	...
	if (Get_Hdmi_hdcp_2_2_Write_Message()) {
		...
		if (HDMI_HDCP_CR_get_hdcp_en(hdmi_rx_reg_read32(HDMI_HDCP_CR, HDMI_RX_MAC))) {
			hdmi_rx_reg_mask32(HDMI_HDCP_CR, ~HDMI_HDCP_CR_hdcp_en_mask, 0, HDMI_RX_MAC);
			HDMIRX_INFO("[HDCP2.2] Discover 2.2, disable 1.4");
		}
		/* Open hdcp2p2 */
		hdmi_rx_reg_mask32(HDMI_HDCP_2P2_CR, ~HDMI_HDCP_2P2_CR_hdcp_2p2_en_mask,
			HDMI_HDCP_2P2_CR_hdcp_2p2_en_mask, HDMI_RX_MAC);
		...
		case AKE_INIT:
			...
			/* Discover HDCP 2.2 */
			hdmirx_state.hdcp_state = HDCPRX_STATE_2P2_DISCOVER;
		...
		case SKE_Send_Eks:
			...
			hdmi_rx_reg_mask32(HDMI_HDCP_2P2_CR, ~HDMI_HDCP_2P2_CR_switch_state_mask,
				HDMI_HDCP_2P2_CR_switch_state(0x01), HDMI_RX_MAC);
			hdmi_rx_reg_mask32(HDMI_HDCP_2P2_CR, ~HDMI_HDCP_2P2_CR_apply_state_mask,
				HDMI_HDCP_2P2_CR_apply_state_mask, HDMI_RX_MAC);
			bLen = 24;
			break;
		...
		if (g_bmsg_id != REPEATERAUTH_STREAM_MANAGE) {
			for (i = 1; i <= bLen; i++)
				TX_msg_id[i] = hdmi_rx_reg_read32(HDMI_HDCP_MSDP, HDMI_RX_MAC);
		} else {
			stream_manage_flag = 1;
			stream_manage_msg[0] = g_bmsg_id;
			for (i = 1; i <= 7; i++)
				stream_manage_msg[i] = hdmi_rx_reg_read32(HDMI_HDCP_MSDP, HDMI_RX_MAC);
		}
		...
		/* Clear interrupt */
		hdmi_rx_reg_write32(HDMI_HDCP_2P2_SR0, HDMI_HDCP_2P2_SR0_irq_wr_msg_done_mask, HDMI_RX_MAC);
...
{
	hdmi_rx_reg_mask32(HDMI_HDCP_2P2_CR, ~HDMI_HDCP_2P2_CR_switch_state_mask,
		HDMI_HDCP_2P2_CR_switch_state(0x10), HDMI_RX_MAC); /* authenticated */
	hdmi_rx_reg_mask32(HDMI_HDCP_2P2_CR, ~HDMI_HDCP_2P2_CR_apply_state_mask,
		HDMI_HDCP_2P2_CR_apply_state_mask, HDMI_RX_MAC);
