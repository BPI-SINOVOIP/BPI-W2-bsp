...
void Generate_eMMCBootArrayFWTable(){
	...
	/****************Boot Partition's entries *****************/

	//NOCS Certificate
	block_no = 0;// start from boot partition beginning
	entry.type = SCS_NOCS_Certificate;
	entry.flash_addr = block_no * EMMC_BLOCK_SIZE;
	entry.length = nocs_certificate_size;
	entry.emmc_partition = 1;
	...
	//SCS_Params_Area
	entry.type = SCS_Params_Area;
	entry.flash_addr = block_no * EMMC_BLOCK_SIZE;
	entry.length = scs_params_area_size;
	entry.emmc_partition = 1;
	w_ret += 16 * fwrite(&entry, 16, 1, ptr_eMMC_FW_Table);
	block_no = (entry.flash_addr / EMMC_BLOCK_SIZE) + align_to_boundary(entry.length, EMMC_BLOCK_SIZE);
...
void Generate_SPIBootArrayFWTable(){
	...
	//NOCS Certificate
	entry.type = SCS_NOCS_Certificate;
	entry.flash_addr = SPI_ENV_PARAM_ADDR + SPI_MAX_PARAM_SIZE+ 0x400;
	entry.length = nocs_certificate_size;;
