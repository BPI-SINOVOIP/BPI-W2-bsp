/*
 * apw8889-regulator.c - Anpec APW8889 Regulator sriver
 *
...
static int apw8889_regulator_probe(struct platform_device *pdev)
{
	...
	regcache_cache_bypass(ardev->regmap, true);
	ret = regmap_write(ardev->regmap, APW8889_REG_DISCHG, 0xfd);
	if (ret)
		dev_warn(dev, "failed to reset dischg: %d\n", ret);
	regcache_cache_bypass(ardev->regmap, false);
