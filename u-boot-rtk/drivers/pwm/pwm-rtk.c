...
static struct rtk_pwm_chip rtd_pc, *pc;
...
void _rtk_pwm_pinmux(int id, int loc, int en)
{
	...
	DBG("%s:%d: id %d, loc %d, en %d\n", __func__, __LINE__, id, loc, en);
	switch (pin) {
	case PIN_00:
...
void _rtk_pwm_pinmux(int id, int loc, int en)
{
	...
	case PIN_00:
		value = rtd_inl(ISO_MUXPAD1);
		value &= ~ISO_MUXPAD1_gpio_12_mask;
		if (en)
			value |= ISO_MUXPAD1_gpio_12(ISO_MUXPAD1_PWM_FUNC);
		rtd_outl(ISO_MUXPAD1, value);
		break;

	case PIN_01:
		value = rtd_inl(ISO_MUXPAD1);
		value &= ~ISO_MUXPAD1_gpio_20_mask;
		if (en)
			value |= ISO_MUXPAD1_gpio_20(4);
		rtd_outl(ISO_MUXPAD1, value);
		break;

	case PIN_10:
		value = rtd_inl(ISO_MUXPAD1);
		value &= ~ISO_MUXPAD1_gpio_13_mask;
		if (en)
			value |= ISO_MUXPAD1_gpio_13(ISO_MUXPAD1_PWM_FUNC);
		rtd_outl(ISO_MUXPAD1, value);
		break;

	case PIN_11:
		value = rtd_inl(ISO_MUXPAD1);
		value &= ~ISO_MUXPAD1_gpio_21_mask;
		if (en)
			value |= ISO_MUXPAD1_gpio_21(ISO_MUXPAD1_PWM_FUNC);
		rtd_outl(ISO_MUXPAD1, value);
		break;

	case PIN_20:
		value = rtd_inl(ISO_MUXPAD1);
		value &= ~ISO_MUXPAD1_gpio_14_mask;
		if (en)
			value |= ISO_MUXPAD1_gpio_14(ISO_MUXPAD1_PWM_FUNC);
		rtd_outl(ISO_MUXPAD1, value);
		break;

	case PIN_21:
		value = rtd_inl(ISO_MUXPAD1);
		value &= ~ISO_MUXPAD1_gpio_22_mask;
		if (en)
			value |= ISO_MUXPAD1_gpio_22(ISO_MUXPAD1_PWM_FUNC);
		rtd_outl(ISO_MUXPAD1, value);
		break;

	case PIN_30:
		value = rtd_inl(ISO_MUXPAD1);
		value &= ~ISO_MUXPAD1_gpio_15_mask;
		if (en)
			value |= ISO_MUXPAD1_gpio_15(ISO_MUXPAD1_PWM_FUNC);
		rtd_outl(ISO_MUXPAD1, value);
		break;

	case PIN_31:
		value = rtd_inl(ISO_MUXPAD1);
		value &= ~ISO_MUXPAD1_gpio_23_mask;
		if (en)
			value |= ISO_MUXPAD1_gpio_23(ISO_MUXPAD1_PWM_FUNC);
		rtd_outl(ISO_MUXPAD1, value);
		break;
	}
}
...
void _rtk_pwm_pinmux(int id, int loc, int en)
{
	...
	case PIN_00:
		value = rtd_inl(ISO_MUXPAD0);
		value &= ~ISO_MUXPAD0_gpio_12_mask;
		if (en)
			value |= ISO_MUXPAD0_gpio_12(ISO_MUXPAD0_PWM_FUNC);
		rtd_outl(ISO_MUXPAD0, value);
		break;
	...
	case PIN_01:
		...
		value &= ~ISO_MUXPAD1_gpio_20_mask;
		if (en)
			value |= ISO_MUXPAD1_gpio_20(ISO_MUXPAD1_PWM_FUNC);
		rtd_outl(ISO_MUXPAD1, value);
		break;

	case PIN_10:
		value = rtd_inl(ISO_MUXPAD0);
		value &= ~ISO_MUXPAD0_gpio_13_mask;
		if (en)
			value |= ISO_MUXPAD0_gpio_13(ISO_MUXPAD0_PWM_FUNC);
		rtd_outl(ISO_MUXPAD0, value);
		break;
	...
	case PIN_20:
		value = rtd_inl(ISO_MUXPAD0);
		value &= ~ISO_MUXPAD0_gpio_14_mask;
		if (en)
			value |= ISO_MUXPAD0_gpio_14(ISO_MUXPAD0_PWM_FUNC);
		rtd_outl(ISO_MUXPAD0, value);
		break;
	...
	case PIN_30:
		value = rtd_inl(ISO_MUXPAD0);
		value &= ~ISO_MUXPAD0_gpio_15_mask;
		if (en)
			value |= ISO_MUXPAD0_gpio_15(ISO_MUXPAD0_PWM_FUNC);
		rtd_outl(ISO_MUXPAD0, value);
		break;
...
static void rtk_pwm_pin_mux(void)
{
	#if defined(CONFIG_RTD1395) || defined(CONFIG_RTD161x)
	u32 value = 0;
	u32 mask = 0;
	#endif /* CONFIG_RTD1395 | CONFIG_RTD161x */

	#ifdef PWM_0_PIN_0
	_rtk_pwm_pinmux(0, 0, 1);
	#endif /* PWM_0_PIN_0 */

	#ifdef PWM_0_PIN_1
	_rtk_pwm_pinmux(0, 1, 1);
	#endif /* PWM_0_PIN_1 */

	#ifdef PWM_1_PIN_0
	_rtk_pwm_pinmux(1, 0, 1);
	#endif /* PWM_1_PIN_0 */

	#ifdef PWM_1_PIN_1
	_rtk_pwm_pinmux(1, 1, 1);
	#endif /* PWM_1_PIN_1 */

	#ifdef PWM_2_PIN_0
	_rtk_pwm_pinmux(2, 0, 1);
	#endif /* PWM_2_PIN_0 */

	#ifdef PWM_2_PIN_1
	_rtk_pwm_pinmux(2, 1, 1);
	#endif /* PWM_2_PIN_1 */

	#ifdef PWM_3_PIN_0
	_rtk_pwm_pinmux(3, 0, 1);
	#endif /* PWM_3_PIN_0 */

	#ifdef PWM_3_PIN_1
	_rtk_pwm_pinmux(3, 1, 1);
	#endif /* PWM_3_PIN_1 */

	#if defined(CONFIG_RTD1395) || defined(CONFIG_RTD161x)
	value = rtd_inl(ISO_MUXPAD6);
	mask =	ISO_MUXPAD6_pwm_01_open_drain_en_loc0_mask |
		ISO_MUXPAD6_pwm_23_open_drain_en_loc0_mask |
		ISO_MUXPAD6_pwm_01_open_drain_en_loc1_mask |
		ISO_MUXPAD6_pwm_23_open_drain_en_loc1_mask;
	value &= ~mask;
	value |= (PWM_OPEN_DRAIN_EN << \
			ISO_MUXPAD6_pwm_01_open_drain_en_loc0_shift) &
			mask;
	rtd_outl(ISO_MUXPAD6, value);
	#endif /* CONFIG_RTD1395 | CONFIG_RTD161x */
}
...
int rtk_pwm_init(void)
{
	...
	/* init PWM reg */
	for (i=0; i<NUM_PWM; i++) {
		DBG("--- debug : rtk_pwm_init - hwpwm=(%d) enable=(%d) duty_rate=(%d) clksrc_div=(%d) clkout_div=(%d)---  \n",
			i, pc->enable[i], pc->duty_rate[i], pc->clksrc_div[i], pc->clkout_div[i]);
		DBG("--- debug : rtk_pwm_init - defualt output frequence = %dHz ---  \n",
			pc->out_freq[i]);
		pwm_set_register(i);
	}

	DBG("--- debug : rtk_pwm_init --  done! !\n");
