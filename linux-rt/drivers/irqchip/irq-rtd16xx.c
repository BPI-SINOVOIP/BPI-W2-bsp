/*
 * irq-rtd16xx.c - RTK irq mux driver
 *
...
static int mux_set_type(struct irq_data *d, unsigned int type)
{
	/*
	 * TODO: configure ISO/MSIC interrupt type
	*/
	return 0;
}
...
static int mux_irq_domain_translate(struct irq_domain *d,
	...
	unsigned int *type)
{
	...
	*type = fwspec->param[1];
	...
	virq = irq_find_mapping(d, fwspec->param[0]);
	irq_set_irq_type(virq, fwspec->param[1]);
...
static const struct irq_domain_ops mux_irq_domain_ops = {
	.translate = mux_irq_domain_translate,
