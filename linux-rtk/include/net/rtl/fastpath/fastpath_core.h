...
int rtk_addV6Connection(struct V6_Tuple *PathEntry, struct sk_buff *pskb,
	void *ct);
...
void register_customRspHook(int *_cusRsp401_func, int *_cusRspTCPFinAck_func,
	int *_cusRspTCPEndAck_func);
...
extern int init_V6_table_path(int v6_path_tbl_list_max,
	int v6_path_tbl_entry_max);
...
/* ---------------------------------------------------------------------------------------------------- */
#if 0
enum LR_RESULT rtk_addInterface(uint8 *ifname, ipaddr_t ipAddr,
	ether_addr_t *gmac, uint32 mtu, enum IF_FLAGS flags);
enum LR_RESULT rtk_configInterface(uint8 *ifname, uint32 vlanId, uint32 fid,
	uint32 mbr, uint32 untag, enum IF_FLAGS flags);
enum LR_RESULT rtk_delInterface(uint8 *ifname);
enum LR_RESULT rtk_addFdbEntry(uint32 vid, uint32 fid, ether_addr_t *mac,
	uint32 portmask, enum FDB_FLAGS flags);
...
enum LR_RESULT rtk_modifyArp(ipaddr_t ip, ether_addr_t *mac,
	enum ARP_FLAGS flags);
enum LR_RESULT rtk_delArp(ipaddr_t ip);
enum LR_RESULT rtk_addRoute(ipaddr_t ip, ipaddr_t mask, ipaddr_t gateway,
	uint8 *ifname, enum RT_FLAGS flags);
enum LR_RESULT rtk_modifyRoute(ipaddr_t ip, ipaddr_t mask, ipaddr_t gateway,
	uint8 *ifname, enum RT_FLAGS flags);
enum LR_RESULT rtk_delRoute(ipaddr_t ip, ipaddr_t mask);
enum LR_RESULT rtk_addSession(uint8 *ifname, enum SE_TYPE seType,
	uint32 sessionId, enum SE_FLAGS flags);
...
#if defined(IMPROVE_QOS)
enum LR_RESULT rtk_idleNaptConnection(rtl_fp_napt_entry * fpNaptEntry,
	uint32 interval);
#else
enum LR_RESULT rtk_idleNaptConnection(rtl_fp_napt_entry * fpNaptEntry,
	uint32 interval, unsigned long *lastUsed);
#endif
...
int32 rtl_fp_dev_hard_start_xmit_check(struct sk_buff *skb,
...
#endif
int fast_path_before_nat_check(struct sk_buff *skb, struct iphdr *iph,
	uint32 iphProtocol);
...
void fastpath_set_qos_mark(struct sk_buff *skb, unsigned int preRouteMark,
	unsigned int postRouteMark);
int fast_path_pre_process_check(struct iphdr *iph, struct tcphdr *tcphupuh,
	struct sk_buff *skb);
int fast_path_post_process_xmit_check(struct iphdr *iph,
	struct tcphdr *tcphupuh, struct sk_buff *skb);
int fast_path_post_process_return_check(struct iphdr *iph,
	struct tcphdr *tcphupuh, struct sk_buff *skb);
...
extern enum LR_RESULT (*FastPath_hook2) (ipaddr_t ip, ipaddr_t mask,
	ipaddr_t gateway, uint8 *ifname, enum RT_FLAGS flags);
extern int (*fast_path_hook) (struct sk_buff **pskb);
extern enum LR_RESULT (*FastPath_hook3) (ipaddr_t ip, ipaddr_t mask,
	ipaddr_t gateway, uint8 *ifname, enum RT_FLAGS flags);
extern enum LR_RESULT (*FastPath_hook4) (rtl_fp_napt_entry *fpNaptEntry);
extern enum LR_RESULT (*FastPath_hook5) (ipaddr_t ip, ether_addr_t *mac,
	enum ARP_FLAGS flags);
#if defined(CONFIG_OPENWRT_SDK)
...
extern enum LR_RESULT (*FastPath_hook8) (ipaddr_t ip, ether_addr_t *mac,
	enum ARP_FLAGS flags);
...
#endif
extern enum LR_RESULT (*FastPath_hook11) (rtl_fp_napt_entry *fpNaptEntry,
	uint32 interval);
#if defined(CONFIG_RTL_HW_QOS_SUPPORT) && defined(CONFIG_OPENWRT_SDK)
extern int (*qosGetSkbMarkByNaptEntryHook) (rtl865x_napt_entry *naptEntry,
	rtl865x_qos_mark *qosMark, struct sk_buff *pskb);
#endif
...
#endif
#if !defined(CONFIG_OPENWRT_SDK)
void mark_path_invalid(uint32 sIp, uint16 sPort, uint32 dIp, uint16 dPort,
	uint16 iphProtocol);
#endif
...
extern struct sk_buff *ppp_receive_nonmp_frame(struct ppp *ppp,
	struct sk_buff *skb, int is_fast_fw);
...
void rtl_set_skb_network_header(struct sk_buff *skb,
...
void rtl_set_skb_transport_header(struct sk_buff *skb,
...
unsigned int rtl_ipt_do_table(struct sk_buff *skb, unsigned int hook, void *in,
	void *out);
#if defined(CONFIG_OPENWRT_SDK)
int rtl_ip_route_input(struct sk_buff *skb, __be32 daddr, __be32 saddr, u8 tos);

#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
int rtl_skb_dst_check(struct sk_buff *skb, unsigned long dip, void *ct,
	uint8 type);
#else
...
__be16 rtl_get_ct_port_by_dir(void *ct_ptr, enum ip_conntrack_dir dir,
	int flag);
#if defined(CONFIG_RTD_1295_HWNAT) && LINUX_VERSION_CODE >= KERNEL_VERSION(4,9,0)
void rtl_set_ct_timeout_expires(void *ct_ptr, unsigned int value);
#else
...
#if !defined(CONFIG_OPENWRT_SDK)
void rtl_skb_policy_route_mark_set(struct sk_buff *skb,
	unsigned int policy_route_mark, unsigned int *mark_bak);
void rtl_recheck_fp_policy_route_mark(struct sk_buff *skb,
	uint32 *fp_policy_route_mark);
#endif
...
struct sk_buff *rtl_ppp_receive_nonmp_frame(struct ppp *ppp,
	struct sk_buff *skb, int is_fast_fw);
...
void rtl_set_entry_dscp_out(struct Path_List_Entry *entry_path,
	struct sk_buff *pskb);
void rtl_set_entry_dscp_in(struct Path_List_Entry *entry_path,
	struct sk_buff *pskb);
#endif /* CONFIG_OPENWRT_SDK */
#endif
...
void rtl_set_skb_remark_vlanpri(struct Path_List_Entry *entry_path,
	struct sk_buff *skb);
void rtl_set_entry_vlanpri(struct Path_List_Entry *entry_path,
	struct sk_buff *pskb);
...
void rtl_set_entry_vlanid_in(struct Path_List_Entry *entry_path,
	struct sk_buff *skb);
void rtl_set_entry_vlanid_out(struct Path_List_Entry *entry_path,
	struct sk_buff *skb);
#endif

__u8 rtl_get_skb_phy_port(struct sk_buff *skb);
void rtl_set_skb_phy_port(struct sk_buff *skb, uint8 phy_port);
void rtl_set_skb_src_phy_port(struct sk_buff *skb, unsigned char *resMac,
	int is_wan);
#endif /* !CONFIG_OPENWRT_SDK */

void rtl_conntrack_drop_check_hook(struct nf_conn *ct_tmp, uint16 ipprotocol,
	struct nf_conn *ct);
...
void rtl_get_ct_v6_ip_by_dir(void *ct_ptr, enum ip_conntrack_dir dir, int flag,
...
int rtl_ipt6_do_table(struct sk_buff *skb, unsigned int hook, void *in,
	void *out);
