...
int getISOGPIO(int ISOGPIO_NUM)
{
	...
	if(ISOGPIO_NUM <= 31) {
	...
	else if(ISOGPIO_NUM >= 32 && ISOGPIO_NUM <= 63) {
		...
		if(regValue & (0x01 << bitOffset))
			...
			return 0;
	} else if (ISOGPIO_NUM >= 64 && ISOGPIO_NUM <= 85) {
		bitOffset = ISOGPIO_NUM - 64;

		// Set Direction to Input
		regAddr = (ISO_GPDIR_2_reg);
		regValue = rtd_inl((volatile unsigned int *)(uintptr_t)regAddr);
		regValue = regValue & (~(0x01 << bitOffset));
		rtd_outl((volatile unsigned int *)(uintptr_t)regAddr, regValue);

		// Get Value
		regAddr = (ISO_GPDATI_2_reg);
		regValue = rtd_inl((volatile unsigned int *)(uintptr_t)regAddr);
		if(regValue & (0x01 << bitOffset))
			return 1;
		else
...
{
	if((ISOGPIO_NUM<0)||(ISOGPIO_NUM>85))
	{
