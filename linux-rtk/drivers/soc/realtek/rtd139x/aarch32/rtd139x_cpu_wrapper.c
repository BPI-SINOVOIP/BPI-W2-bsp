...
{
	u32 sctlr;
	sctlr =  read_sysreg(SCTLR);

	/* if cache isn't enabled no need to disable */
	if (!(sctlr & CR_C && !(sctlr & CR_M)))
		return;

	write_sysreg(sctlr & ~(CR_C|CR_M), SCTLR);
...
static int __init scpu_wrapper_init(void)
{
	...
	pr_info("scpu wrappee irq %d, scpu_wrap_addr 0x%08x\n", irq, (u32)scpu_wrap_addr);
	...
	pr_info("[SCPU_WRAPPER] initialized\n");

	pr_info("[SCPU_WRAPPER][%s] memory monitor 0x98013b00 - 0x98013c00\n", __func__);
