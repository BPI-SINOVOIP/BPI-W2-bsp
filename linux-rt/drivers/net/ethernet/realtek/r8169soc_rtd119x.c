...
static void rtl8169_tx_desc_dump(struct rtl8169_private *tp)
{
        int i = 0;

        for (i = 0; i < NUM_TX_DESC; i++) {
		printk(KERN_INFO "Desc[%d] opts1=%x opts2=%x addr= %llx\n",i,tp->TxDescArray[i].opts1,tp->TxDescArray[i].opts2,tp->TxDescArray[i].addr);                
        }
}


static void rtl8169_tx_desc_clear(struct rtl8169_private *tp)
{
        int i = 0;
...
static void rtl8169_rx_desc_offset0_init(struct rtl8169_private *tp, int own)
{
	int i = 0;
	int ownbit = 0;
	...
	for (i = 0; i < NUM_RX_DESC; i++) {
		if (i == (NUM_RX_DESC - 1))
			tp->RxDescArray[i].opts1 = cpu_to_le32((ownbit | RingEnd) | (unsigned long)rx_buf_sz);
		else
			tp->RxDescArray[i].opts1 = cpu_to_le32(ownbit | (unsigned long)rx_buf_sz);
	}
}
...
static void rtl_reset_work(struct rtl8169_private *tp)
{
	...
	rtl8169_rx_desc_offset0_init(tp,1);	

	rtl8169_tx_clear(tp);
	
	// prevent Tx desc is overwriten
	rtl8169_tx_desc_clear(tp);
	rtl8169_init_ring_indexes(tp);
