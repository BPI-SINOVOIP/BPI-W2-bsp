               ...
               const unsigned int page_offset)
{
        ...
        unsigned char *loc;
#ifdef CONFIG_VERIFY_SPI
        unsigned int array_size = rem_size;
        unsigned char *src = array_ptr;
#endif

	if (((uintptr_t)dest + rem_size) > (SPI_BASE_ADDR + device->size)) {
		prints("error: image exceed flash size\n");
		return -1;
	}
	...
	sys_dcache_flush_MVA(array_ptr, rem_size);
        ...
        if (compare_memory(src, dest, array_size) != 0) {
                prints("fail\n");
                hexdump("src", src, 0x40);
                hexdump("dest", dest, 0x40);
                return -1;
        }
	else {
                prints("ok\n");
