...
typedef struct rtl_nf_conntrack_info {
	struct net				*net;
	struct nf_conn				*ct;
	struct sk_buff 			*skb;
	...
	u_int32_t			hash;
	u_int32_t			reply_hash;
	void (*double_unlock)(struct rtl_nf_conntrack_info *conn_info);
	void (*double_lock)(struct rtl_nf_conntrack_info *conn_info);
...
#if IS_ENABLED(CONFIG_IP_VS_PROTO_TCP)
...
#if IS_ENABLED(CONFIG_IP_VS_PROTO_UDP)
